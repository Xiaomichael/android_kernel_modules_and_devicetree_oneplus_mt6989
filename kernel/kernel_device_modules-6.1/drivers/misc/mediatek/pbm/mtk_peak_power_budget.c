...
static void bat_handler(struct work_struct *work)
{
	...
	if (ret)
		...
		if (strcmp(fg_data.psy_name, "mtk-gauge") == 0)
			qmax = val.intval;
		else
			qmax = val.intval * 98 / 10;
	}
	if (mt_ppb_debug)
		pr_info("%s: ret=%d, qmax=%d\n", __func__, ret, val.intval);
...
static int read_power_budget_dts(struct platform_device *pdev)
{
	...
	memset(fg_data.psy_name, 0, PSY_NAME_MAX_LEN);
	ret = of_property_read_string(np, "psy_name", &psy_name);

	/*
	 * use the battery as psy_name.
	 * Note: when use mtk-gauge, user shall config the psy_name in the dts.
	*/
	if (ret < 0) {
		pr_info("%s:%d: read psy_name error %d\n", __func__, __LINE__, ret);
		snprintf(fg_data.psy_name, sizeof("battery"), "battery");
	} else {
		snprintf(fg_data.psy_name, sizeof(fg_data.psy_name), "%s", psy_name);
	}
