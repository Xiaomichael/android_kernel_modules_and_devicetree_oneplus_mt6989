...
{
    enum PANEL_ES panel_es_ver = ES_EVT;
    switch (m_db) {
        case 1:
            panel_es_ver = ES_T0;
            break;
...
static void lcm_panel_init(struct lcm *ctx)
{
    ...
    pr_info("sdc_nt37708s %s +, mode id=%d panel_es_ver = %d\n", __func__, mode_id, panel_es_ver);
    switch (mode_id) {
        case FHD_SDC60:
            if (panel_es_ver == ES_T0) {
                push_table(ctx, init_setting_60Hz,
                           sizeof(init_setting_60Hz) / sizeof(struct LCM_setting_table));
            } else {
                push_table(ctx, init_setting_60Hz_EVT,
                           sizeof(init_setting_60Hz_EVT) / sizeof(struct LCM_setting_table));
            }
            break;
        case FHD_SDC90:
        ...
        case FHD_SDC120:
            ...
            break;
        case FHD_SDC144:
            push_table(ctx, init_setting_144Hz, sizeof(init_setting_144Hz)/sizeof(struct LCM_setting_table));
            break;
        default:
            push_table(ctx, init_setting_60Hz, sizeof(init_setting_60Hz)/sizeof(struct LCM_setting_table));
            break;
    }
    push_table(ctx, init_common, sizeof(init_common)/sizeof(struct LCM_setting_table));
    if (panel_es_ver == ES_T0) {
        push_table(ctx, init_T0_ext, sizeof(init_T0_ext)/sizeof(struct LCM_setting_table));
    } else {
        push_table(ctx, init_EVT_ext, sizeof(init_EVT_ext)/sizeof(struct LCM_setting_table));
    }
    push_table(ctx, init_11_29, sizeof(init_11_29)/sizeof(struct LCM_setting_table));
                       ...
                       void *handle, unsigned int hbm_mode)
{
    ...
    OFP_INFO("sdc_nt37708s lcm_set_hbm oplus_display_brightness=%d, hbm_mode=%u\n", oplus_display_brightness, hbm_mode);
...
static int oplus_ofp_set_lhbm_pressed_icon(struct drm_panel *panel, void *dsi_drv, dcs_write_gce_pack cb, void *handle, bool lhbm_pressed_icon_on)
{
    ...
    OFP_INFO("sdc_nt37708s lhbm_pressed_icon_on:%u,bl_lvl:%u,refresh_rate:%u\n", lhbm_pressed_icon_on, level, vrefresh_rate);
    ...
    if (lhbm_pressed_icon_on) {
        if (level < 2900) {
            ...
            g_fpr_ratio = 11624;
            b_fpr_ratio = 11394;
        } else if (level >= 2900 && level < 3102) {
            r_fpr_ratio = 10721;
            g_fpr_ratio = 11133;
            b_fpr_ratio = 10915;
        } else if (level >= 3102 && level < 3338) {
            r_fpr_ratio = 10015;
            g_fpr_ratio = 10382;
            b_fpr_ratio = 10171;
        } else if (level >= 3338 && level < 3573) {
            r_fpr_ratio = 9292;
            g_fpr_ratio = 9668;
            b_fpr_ratio = 9475;
        } else if (level >= 3573 && level < 3825) {
            r_fpr_ratio = 8741;
            g_fpr_ratio = 9072;
            b_fpr_ratio = 8888;
        } else if (level >= 3825) {
                       ...
                       unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
    ...
    if (drm_mode_vrefresh(m) == 60) {
        ...
        pr_info("sdc_nt37708s %s timing switch to 144\n", __func__);
        push_table(ctx, mode_switch_to_144, sizeof(mode_switch_to_144) / sizeof(struct LCM_setting_table));
        ret = 1;
    }
    ctx->m = m;
    return ret;
                         ...
                         struct drm_connector *connector)
{
    ...
    pr_info("sdc_nt37708s %s clock=%d,htotal=%d,vtotal=%d,hskew=%d,vrefresh=%d\n", __func__, mode[0]->clock, mode[0]->htotal,
