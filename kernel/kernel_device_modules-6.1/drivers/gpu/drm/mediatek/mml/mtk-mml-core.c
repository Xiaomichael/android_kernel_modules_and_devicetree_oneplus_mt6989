...
static u32 mml_core_calc_tput_couple(struct mml_task *task, u32 pixel, u32 pipe)
{
	...
	tput_debug[0] = task->pipe[pipe].throughput;
	...
	if (info->mode == MML_MODE_RACING) {
		...
		tput_debug[1] = task->pipe[pipe].throughput;
	...
	if (info->mode == MML_MODE_RACING && cfg->dual) {
		out_w[0] = (cfg->panel_w / 2) - cfg->dl_out[0].left;
		out_w[1] = cfg->dl_out[1].width - (cfg->dl_out[0].width - out_w[0]);

		if (out_w[pipe])
			task->pipe[pipe].throughput = task->pipe[pipe].throughput * (cfg->panel_w / 2) / out_w[pipe];
	}
	tput_debug[2] = task->pipe[pipe].throughput;

	mml_msg_qos("%s job %u pipe %u pixel %u act_time %u tput (%u %u %u) width (%u %u %u)", __func__,
		task->job.jobid, pipe, pixel, act_time_us,
		tput_debug[0], tput_debug[1], tput_debug[2],
		cfg->panel_w, out_w[0], out_w[1]);
