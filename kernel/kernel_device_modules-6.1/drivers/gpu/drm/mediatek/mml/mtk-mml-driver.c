	...
	struct mml_comp_config *ccfg, u32 throughput, u32 tput_up)
{
	...
	if (!datasize) {
...
#else
			if (cfg->info.mode == MML_MODE_RACING && cfg->dual) {
				out_w[0] = (cfg->panel_w / 2) - cfg->dl_out[0].left;
				out_w[1] = cfg->dl_out[1].width - (cfg->dl_out[0].width - out_w[0]);

				if ((out_w[0] + out_w[1]) && (cfg->panel_w > (out_w[0] + out_w[1])))
					hrt_bw = (u32)((u64)hrt_bw * cfg->panel_w / (out_w[0] + out_w[1]));
			}
		...
		mml_msg_qos("%s job %u comp %u srt_bw %u hrt_bw %u datasize %u act_time %u panel_w %u dest_w %u %u %u",
			__func__, jobid, comp->id, bandwidth, hrt_bw,
			datasize, cfg->info.act_time,
			cfg->panel_w, dest->data.width, out_w[0], out_w[1]);
	} else {
		hrt = false;
