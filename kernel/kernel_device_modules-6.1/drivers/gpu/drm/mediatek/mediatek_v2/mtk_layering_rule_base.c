			   ...
			   int debug_mode, int disp_idx)
{
	...
	if (iris_is_chip_supported())
		l_info->input_config[disp_idx] = kvzalloc(layer_size, GFP_KERNEL);
	else
...
#if defined(CONFIG_PXLW_IRIS)
	if (iris_is_chip_supported())
		l_info->mml_cfg[disp_idx] = kvzalloc(mml_cfg_size, GFP_KERNEL);
	else
...
#if defined(CONFIG_PXLW_IRIS)
		if (iris_is_chip_supported()) {
			kvfree(l_info->input_config[disp_idx]);
			kvfree(l_info->mml_cfg[disp_idx]);
		} else {
	...
	for (i = 0; i < disp_info->layer_num[disp_idx]; i++) {
		...
		if (MTK_MML_DISP_DIRECT_DECOUPLE_LAYER & c->layer_caps) {
			...
			if (panel_is_dunhuang()) {
				c->layer_caps &= ~MTK_MML_DISP_DIRECT_DECOUPLE_LAYER;
				c->layer_caps |= MTK_MML_DISP_DECOUPLE_LAYER;
				DDPINFO("%s hrt_idx:%d unable to get MML_IR, set to MML_DC\n",
					__func__, hrt_idx);
			}
		}
