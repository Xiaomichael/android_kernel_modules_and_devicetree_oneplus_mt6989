...
EXPORT_SYMBOL(panel_is_dunhuang);

bool panel_is_greenland(void)
{
	int prj_id = get_project();
	if (prj_id == 25921 || prj_id == 25925 || prj_id == 25971 ||
			 prj_id == 25972 || prj_id == 25973 || prj_id == 25974)
		return true;
	return false;
}
EXPORT_SYMBOL(panel_is_greenland);

bool panel_is_chrome(void)
{
	int prj_id = get_project();
	if (prj_id == 25682 || prj_id == 25683 || prj_id == 25701)
		return true;
	return false;
}
EXPORT_SYMBOL(panel_is_chrome);

bool panel_is_casioy_nt37706a(void)
{
	int prj_id = get_project();
	if (prj_id == 24780)
		return true;
...
EXPORT_SYMBOL(panel_is_casioy_nt37706a);
...
#if defined(CONFIG_PXLW_IRIS)
void mtk_dsi_write_relaxed(struct mtk_dsi *dsi, unsigned int offset, unsigned int data)
{
	writel_relaxed(data, dsi->regs + offset);
}
...
static void oplus_mtk_dsi_dphy_timconfig(struct mtk_dsi *dsi, void *handle)
{
	u32 lpx = 0;
	...
	value = REG_FLD_VAL(FLD_LPX, lpx) | REG_FLD_VAL(FLD_HS_PREP, da_hs_prep)
	| REG_FLD_VAL(FLD_HS_ZERO, da_hs_zero) | REG_FLD_VAL(FLD_HS_TRAIL, da_hs_trail);

	if (handle) {
		cmdq_pkt_write((struct cmdq_pkt *)handle, comp->cmdq_base, comp->regs_pa + DSI_PHY_TIMECON0, value, ~0);
	} else {
		writel(value, dsi->regs + DSI_PHY_TIMECON0);
	}

	value = REG_FLD_VAL(FLD_TA_GO, ta_go) | REG_FLD_VAL(FLD_TA_SURE, ta_sure)
	| REG_FLD_VAL(FLD_TA_GET, ta_get) | REG_FLD_VAL(FLD_DA_HS_EXIT, da_hs_exit);

	if (handle) {
		cmdq_pkt_write((struct cmdq_pkt *)handle, comp->cmdq_base, comp->regs_pa + DSI_PHY_TIMECON1, value, ~0);
	} else {
		writel(value, dsi->regs + DSI_PHY_TIMECON1);
	}

	value = REG_FLD_VAL(FLD_CONT_DET, cont_det) | REG_FLD_VAL(FLD_DA_HS_SYNC, da_hs_sync)
	| REG_FLD_VAL(FLD_CLK_HS_ZERO, clk_hs_zero) | REG_FLD_VAL(FLD_CLK_HS_TRAIL, clk_hs_trail);

	if (handle) {
		cmdq_pkt_write((struct cmdq_pkt *)handle, comp->cmdq_base, comp->regs_pa + DSI_PHY_TIMECON2, value, ~0);
	} else {
		writel(value, dsi->regs + DSI_PHY_TIMECON2);
	}

	value = REG_FLD_VAL(FLD_CLK_HS_PREP, clk_hs_prep) | REG_FLD_VAL(FLD_CLK_HS_POST, clk_hs_post) | REG_FLD_VAL(FLD_CLK_HS_EXIT, clk_hs_exit);

	if (handle) {
		cmdq_pkt_write((struct cmdq_pkt *)handle, comp->cmdq_base, comp->regs_pa + DSI_PHY_TIMECON3, value, ~0);
	} else {
...
{
	if (panel_is_casioy_nt37706a() && dsi->vfp == 116) {
		oplus_mtk_dsi_dphy_timconfig(dsi, handle);
	} else {
...
#if defined(CONFIG_PXLW_IRIS)
	if (iris_is_chip_supported() && mtk_dsi_is_cmd_mode(&dsi->ddp_comp))
...
static void mtk_dsi_calc_vdo_timing(struct mtk_dsi *dsi)
{
	...
	if (dsi->ext->params->is_cphy) {
		...
		if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_SYNC_PULSE) {
			...
			if (panel_is_dunhuang()) {
				...
				DDPINFO("%s hsyn=0x%x chbp=0x%x hfp=0x%x ps_wc=0x%x lanes=%d\n", __func__, horizontal_sync_active_byte,
				...
				DDPINFO("%s v_active_roundup=%d horizontal_frontporch_byte=0x%x \n", __func__,
						v_active_roundup, horizontal_frontporch_byte);
			}
		} else {
			horizontal_sync_active_byte =
			...
			if (panel_is_dunhuang()) {
				if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_BURST) {
					...
					DDPINFO("%s hbp=0x%x hfp=0x%x ps_wc=0x%x bllp_wc=0x%x lanes=%d\n", __func__,
					...
					DDPINFO("%s hbp=0x%x hfp=0x%x ps_wc=0x%x lanes=%d\n", __func__,
							horizontal_backporch_byte, horizontal_frontporch_byte, ps_wc, dsi->lanes);
	...
	if(panel_is_dunhuang() && dsi->slave_dsi) {
...
irqreturn_t mtk_dsi_irq_status(int irq, void *dev_id)
{
	...
	if (status) {
		...
		if ((status & TE_RDY_INT_FLAG) &&
			...
			if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->update_te_time) {
				dsi->ext->funcs->update_te_time();
			}
	...
	if (doze_enabled && panel_funcs->doze_enable_start)
		...
		if (oplus_ofp_get_aod_state() && !oplus_ofp_a_mirror_to_the_end_aod_mode_is_enabled()) {
			if(oplus_ofp_video_mode_30hz_aod_is_enabled()) {
				...
				if (panel_funcs->oplus_ofp_set_lhbm_pressed_icon_single)
					oplus_ofp_doze_status_handle(doze_enabled, dsi->encoder.crtc, dsi->ext, dsi->panel, dsi, mipi_dsi_dcs_write_gce2);
				else
...
static void print_cmd_desc(const struct mipi_dsi_msg *msg)
{
	...
	/* TX len */
	len += snprintf(buf + len, sizeof(buf) - len, "%02x %02x ", (unsigned int)(msg->tx_len >> 8), (unsigned int)(msg->tx_len & 0x00FF));
		...
		if (new_doze_state && !dsi->doze_enabled) {
			...
			if (ext && ext->funcs
				...
				if(oplus_ofp_video_mode_30hz_aod_is_enabled()) {
					...
					if (ext->funcs->oplus_ofp_set_lhbm_pressed_icon_single)
						oplus_ofp_doze_status_handle(new_doze_state, crtc, ext, dsi->panel, dsi, mipi_dsi_dcs_write_gce2);
					else
	...
	int force_lcm_update)
{
	...
	if ((crtc_idx != 0) && (crtc_idx != 3)) {
		DDPINFO("%s:%d skip crtc[%d] wait frame done\n", __func__, __LINE__, crtc_idx);
		return 0;
	}
#endif
...
int mtk_dsi_esd_read(struct mtk_ddp_comp *comp, void *handle, void *ptr)
{
...
#endif
	{
		...
		if (dsi->ext->funcs && dsi->ext->funcs->esd_check_multipage_pre) {
			dsi->ext->funcs->esd_check_multipage_pre(dsi, mipi_dsi_dcs_write_gce_dyn, handle, i);
		}
#endif
	...
	if (iris_is_chip_supported() && iris_get_esd_check_ongoing()) {
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			if (lcm_esd_tb->mask_list[0])
				chk_val[0] = chk_val[0] & lcm_esd_tb->mask_list[0];
...
#endif
	{
		...
		for (j = 0; j < lcm_esd_tb->count && j < 4; j++) {
			...
			if (chk_val[j] == lcm_esd_tb->para_list[j]) {
				...
				lcm_esd_tb->cmd,chk_val[j], lcm_esd_tb->para_list[0], lcm_esd_tb->mask_list[j]);
				ret = 0;
				if(lcm_esd_tb->revert_flag) {
					ret = -1;
					DDPPR_ERR("[DSI]cmp fail:read(0x%x)=expect(0x%x || 0x%x)\n", chk_val[j], lcm_esd_tb->para_list[j], lcm_esd_tb->mask_list[j]);
				}
			} else {
				if(lcm_esd_tb->revert_flag) {
					ret = 0;
					DDPINFO("[DSI]lcm_esd_tb->cmd=0x%x,read(0x%x) = expect(0x%x),(0x%x)\n",
							lcm_esd_tb->cmd, chk_val[j], lcm_esd_tb->para_list[j], lcm_esd_tb->mask_list[j]);
				} else {
					ret = -1;
					DDPPR_ERR("[DSI]cmp fail:lcm_esd_tb->cmd=0x%x,read(0x%x)!=expect(0x%x)\n",
					lcm_esd_tb->cmd, chk_val[j], lcm_esd_tb->para_list[j]);
				}
			}
			if (ret < 0) {
				DDPPR_ERR("[DSI]cmp fail:read(0x%x)!=expect(0x%x)\n",
...
#if defined(CONFIG_PXLW_IRIS)
static void iris_dsi_cmdq(struct mtk_dsi *dsi, const struct mipi_dsi_msg *msg)
{
	const char *tx_buf = msg->tx_buf;
	u8 config, cmdq_size, cmdq_off = 0, type = msg->type;
	u32 cmdq_val;
	u32 reg_val = 0, cmdq_mask, i, reg_cmdq_ofs = dsi->driver_data->reg_cmdq0_ofs;
	...
	if (msg->tx_len > 2) {
		...
		mtk_dsi_write_relaxed(dsi, reg_cmdq_ofs, reg_val);
		...
		for (i = 0; i < msg->tx_len; i++) {
			...
			if (((i & 0x3) == 0x3) || (i == (msg->tx_len - 1))) {
				base_addr = reg_cmdq_ofs + cmdq_off + ((i / 4) * 4);
				mtk_dsi_write_relaxed(dsi, base_addr, reg_val);
				DDPDBG("set cmdq addr %x, val:%x\n",
					base_addr,
					reg_val);
				reg_val = 0;
	...
	if (mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
		...
		if (dsi->slave_dsi) {
			if (dsi->ext->params->lcm_cmd_if == MTK_PANEL_DUAL_PORT) {
				iris_dsi_cmdq_gce(dsi->slave_dsi, handle, msg);
				cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base,
					dsi->slave_dsi->ddp_comp.regs_pa + DSI_CON_CTRL,
					DSI_DUAL_EN, DSI_DUAL_EN);
			} else
...
#if defined(CONFIG_PXLW_IRIS)
       if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
               if (!_iris_dsi_dcs_write_gce(dsi, handle, &msg, data, len, 1))
                       return;
       }
#endif /*CONFIG_PXLW_IRIS*/
...
EXPORT_SYMBOL(mipi_dsi_dcs_write_gce);
					  ...
					  const void *data, size_t len)
{
	...
	struct cmdq_pkt *handle2 = NULL;
	struct cmdq_pkt *handle3 = NULL;
	struct cmdq_pkt *handle4 = NULL;
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(dsi->encoder.crtc);
	int dsi_mode;
	int panel_mode;
	...
	if(!panel_mode){
		...
		cmdq_pkt_flush(handle);
		cmdq_pkt_destroy(handle);
		DDPINFO("%s wfe EVENT_VDO_CABC_EOF done\n", __func__);
	}
	...
	if (dsi_mode == 0) {
		...
		mtk_dsi_power_keep_gce(dsi, handle2, true);
		mtk_dsi_poll_for_idle(dsi, handle2);

#if defined(CONFIG_PXLW_IRIS)
		if (iris_is_chip_supported()) {
			msg.flags = 0;
			iris_dsi_cmdq_gce(dsi, handle2, &msg);
		} else
			mtk_dsi_cmdq_gce(dsi, handle2, &msg);
#else /*CONFIG_PXLW_IRIS*/
		mtk_dsi_cmdq_gce(dsi, handle2, &msg);
#endif
#ifdef OPLUS_FEATURE_DISPLAY
		...
		if (mtk_dsi_is_cmd_mode(&dsi->ddp_comp) && dsi->driver_data->require_phy_reset) {
			/* CMD mode require phy reset only */
			mtk_dsi_runtime_phy_reset_gce(dsi, handle2);
		}

		cmdq_pkt_write(handle2, dsi->ddp_comp.cmdq_base,
			dsi->ddp_comp.regs_pa + DSI_START, 0x0, ~0);
		cmdq_pkt_write(handle2, dsi->ddp_comp.cmdq_base,
			dsi->ddp_comp.regs_pa + DSI_START, 0x1, ~0);
		cmdq_pkt_write(handle2, dsi->ddp_comp.cmdq_base,
		...
		mtk_dsi_poll_for_idle(dsi, handle2);
		mtk_dsi_power_keep_gce(dsi, handle2, false);

		cmdq_pkt_flush(handle2);
		cmdq_pkt_destroy(handle2);
		DDPINFO("%s cmd mode send dsi cmd done\n", __func__);
	} else {
		mtk_crtc_pkt_create(&handle3, &mtk_crtc->base,
			mtk_crtc->gce_obj.client[CLIENT_DSI_CFG]);
...
#endif /* OPLUS_FEATURE_DISPLAY */
		/* build VM cmd */
		mtk_dsi_vm_cmdq(dsi, &msg, handle3);

		/* clear VM_CMD_DONE */
		cmdq_pkt_write(handle3, dsi->ddp_comp.cmdq_base,
			dsi->ddp_comp.regs_pa + DSI_INTSTA, 0,
			VM_CMD_DONE_INT_EN);
		...
		/* poll VM cmd done */
		mtk_dsi_cmdq_poll(&dsi->ddp_comp, handle3,
			dsi->ddp_comp.regs_pa + DSI_INTSTA,
			VM_CMD_DONE_INT_EN, VM_CMD_DONE_INT_EN);
		...
		cmdq_pkt_flush(handle3);
		cmdq_pkt_destroy(handle3);
		DDPINFO("%s vdo mode send dsi cmd done\n", __func__);
	}

	if(!panel_mode){
		mtk_crtc_pkt_create(&handle4, &mtk_crtc->base,
			mtk_crtc->gce_obj.client[CLIENT_DSI_CFG]);
		cmdq_pkt_set_event(handle4,
			mtk_crtc->gce_obj.event[EVENT_VDO_CABC_EOF]);
		cmdq_pkt_flush(handle4);
		cmdq_pkt_destroy(handle4);
		DDPINFO("%s set EVENT_VDO_CABC_EOF done\n", __func__);
	}
			...
			struct mtk_drm_crtc *mtk_crtc, unsigned int cur_mode, unsigned int dst_mode)
{
	...
	for (i = 0; i < MAX_DYN_CMD_NUM; i++) {
		...
		if (dfps_cmd->src_fps == 0 || drm_mode_vrefresh(old_mode) == dfps_cmd->src_fps)
			...
			{
			if (iris_is_chip_supported()) {
				if (iris_vdo_mode_send_panel_cmd(handle, dfps_cmd->para_list,
						dfps_cmd->cmd_num))
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (dsi->ext->params->before_power_down) {
		need_ajust_timing_switch_seq = false;
	}
#endif
...
#if defined(CONFIG_PXLW_IRIS)
	if (iris_is_chip_supported() && (drm_crtc_index(&mtk_crtc->base) == 0))
		iris_switch_mtk(NULL, mtk_crtc, state->prop_val[CRTC_PROP_DISP_MODE_IDX]);
#endif
...
#ifdef OPLUS_FEATURE_DISPLAY
		if (mtk_crtc->skip_mmclk_set_flag && oplus_ofp_video_mode_30hz_aod_is_enabled()
			&& (30 == drm_mode_vrefresh(&mtk_crtc->base.state->adjusted_mode))) {
			DDPPR_ERR("%s skip mmclk set\n", __func__);
			mtk_crtc->skip_mmclk_set_flag = false;
			DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
			goto done;
		}
#endif
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (fps_chg_index & MODE_DSI_HFP) {
		...
		if (dsi && dsi->ext && dsi->ext->params
			&& dsi->mipi_hopping_sta && dsi->ext->params->dyn.vbp) {
			DDPINFO("%s,mipi_clk_change_sta\n", __func__);
...
#if defined(CONFIG_PXLW_IRIS)
		if (iris_is_chip_supported() && (drm_crtc_index(&mtk_crtc->base) == 0))
			iris_switch_fps_with_hfp(handle, mtk_crtc, state->prop_val[CRTC_PROP_DISP_MODE_IDX]);
#endif /* CONFIG_PXLW_IRIS */
...
#if defined(CONFIG_PXLW_IRIS)
		if (iris_is_chip_supported() && (drm_crtc_index(&mtk_crtc->base) == 0))
			iris_switch_fps_with_vfp(handle, mtk_crtc, state->prop_val[CRTC_PROP_DISP_MODE_IDX]);
#endif
			...
			struct mtk_drm_crtc *mtk_crtc, struct mtk_vdo_aod_cfg *vdo_aod_cfg)
{
	...
	if(porch_change_flag & MODE_DSI_HFP){
		...
		mtk_crtc->skip_mmclk_set_flag = !vdo_aod_cfg->aod_en;
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
		...
		if (!panel_ext->params->vdo_mix_mode_en) {
			if (panel_ext && panel_ext->funcs
				...
				if (silence_mode) {
					...
					if (panel_ext->params->set_backlight_delay){
					panel_ext->funcs->set_backlight_cmdq(dsi,
						mipi_dsi_dcs_write_gce_delay,
						handle, *(int *)params);
					} else {
					panel_ext->funcs->set_backlight_cmdq(dsi,
	...
	case OPLUS_OFP_SET_LHBM_PRESSED_ICON:
	{
		...
		} else if (panel_ext && panel_ext->funcs && panel_ext->funcs->oplus_ofp_set_lhbm_pressed_icon_single) {
#ifdef OPLUS_FEATURE_DISPLAY
			if (silence_mode || silence_flag) {
				OFP_INFO("silence_mode is %d, do not set lhbm pressed icon\n", silence_mode);
				break;
...
#if defined(CONFIG_PXLW_IRIS)
static void iris_send_cmd_with_vdo2cmd(struct cmdq_pkt *handle,
		struct mtk_ddp_comp *comp,
		const void *data, size_t len, u32 flag)
{
	...
	if (!handle || !comp || !data) {
		DRM_WARN("IRIS_LOG: handle comp data null return\n");
		return;
	}
...
EXPORT_SYMBOL(iris_dsi_pre_cmd);
...
EXPORT_SYMBOL(iris_dsi_pos_cmd);
...
struct iris_mtk_dsi_op iris_mtk_op = {
	...
	.vdo2cmd_cb = iris_send_cmd_with_vdo2cmd,
};
#endif /* CONFIG_PXLW_IRIS */
