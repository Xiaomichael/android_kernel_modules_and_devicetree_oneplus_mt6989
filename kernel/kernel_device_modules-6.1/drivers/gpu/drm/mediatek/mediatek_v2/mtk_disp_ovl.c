			   ...
			   struct mtk_ddp_config *cfg, struct cmdq_pkt *handle)
{
	...
	mtk_ddp_write(comp, (height * 8) / 10,
		OVL_ROI_TIMING_0, handle);

	DDPINFO("%s -> %u\n", __func__, (height * 8) / 10);
...
enum mtk_drm_dataspace mtk_ovl_map_lcm_color_mode(enum mtk_drm_color_mode cm)
{
...
EXPORT_SYMBOL(mtk_ovl_map_lcm_color_mode);
			...
			struct mtk_plane_state *state, struct cmdq_pkt *handle)
{
	...
	if ((mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_OVL_WCG) &&
		...
		if (fod_status && (fp_type & FOD_COLOR_SUPPORT)) {
			gamma_en = false;
			gamma_sel = 0;
			igamma_en = false;
			igamma_sel = 0;
	...
	case PMQOS_UPDATE_BW: {
		...
		if (per_larb_peak_bw) {
			...
			__mtk_disp_set_module_srt(comp->qos_req, comp->id, comp->qos_bw, 0,
						    DISP_BW_NORMAL_MODE);
			//log+
			if (update_pending && comp->last_qos_bw > comp->qos_bw)
				DDPINFO("DDDDR update %s qos BW pe %u\n", mtk_dump_comp_str(comp), comp->qos_bw);
			else if (comp->last_qos_bw < comp->qos_bw)
				DDPINFO("DDDDR update %s qos BW im %u\n", mtk_dump_comp_str(comp), comp->qos_bw);
			//log-
			comp->last_qos_bw = comp->qos_bw;
			comp->last_hrt_bw = comp->hrt_bw;
		}
...
other:
		if (!IS_ERR(comp->qos_req_other)) {
			if (per_larb_peak_bw) {
				...
				comp->last_hrt_bw_other = comp->hrt_bw_other;
				//log+
				if (update_pending && comp->last_qos_bw_other > comp->qos_bw_other)
					DDPINFO("DDDDR update %s qos other pe %u\n", mtk_dump_comp_str(comp), comp->qos_bw_other);
				else if (comp->last_qos_bw_other < comp->qos_bw_other)
					DDPINFO("DDDDR update %s qos other im %u\n", mtk_dump_comp_str(comp), comp->qos_bw_other);
				//log-
			}
			if (!force_update && !update_pending)
				mtk_crtc->total_srt += comp->qos_bw_other;
