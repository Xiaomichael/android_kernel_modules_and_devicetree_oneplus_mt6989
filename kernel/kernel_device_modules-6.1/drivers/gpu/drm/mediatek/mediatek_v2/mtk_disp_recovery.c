...
extern bool panel_is_greenland(void);
extern bool panel_is_chrome(void);
...
int _mtk_esd_check_read(struct drm_crtc *crtc, int check_num)
{
	...
	if (mtk_dsi_is_cmd_mode(output_comp)) {
		...
		if (!panel_is_dunhuang() || !panel_is_greenland() || !panel_is_chrome())
			cmdq_pkt_wfe(cmdq_handle,
...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	while (1) {
		if (!strcmp(panel_name, "ac158_p_b_a0012_cmd_panel")
			|| panel_is_dunhuang() || panel_is_greenland() || panel_is_chrome()) {
