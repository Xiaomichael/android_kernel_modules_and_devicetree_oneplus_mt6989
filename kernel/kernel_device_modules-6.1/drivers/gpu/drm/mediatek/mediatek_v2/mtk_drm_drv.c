...
void mtk_drm_top_clk_prepare_enable(struct drm_device *drm)
{
	...
	/* check clk prepare enable success or not, force enable all CG if prepare enable fail */
	/* check only one topclk status */
	if ((priv->data->mmsys_id == MMSYS_MT6989) &&
			 ((readl_relaxed(priv->config_regs + 0x100) & 0x1) == 0x1))
		mtk_force_enable_cg_mt6989(priv);
		...
		struct drm_file *file_priv)
{
	...
	ret = iris_drm_operate_conf(data);
	return ret;
}
#endif
...
#ifdef OPLUS_FEATURE_DISPLAY
	pr_info("get_boot_mode() is %d\n", get_boot_mode());
	if ((get_boot_mode() == SILENCE_BOOT)
		||(get_boot_mode() == OPLUS_SAU_BOOT)) {
		DDPINFO("%s OPLUS_SILENCE_BOOT set silence_mode to 1\n", __func__);
		silence_mode = 1;
	}
#endif
