...
#ifdef OPLUS_TRACKPOINT_REPORT
		if (aal_err_cnt > 0) {
			display_exception_trackpoint_report("DisplayDriverID@@510$$ disp_aal_config_mdp_aal");
			aal_err_cnt--;
		}
#endif
...
#ifdef OPLUS_FEATURE_DISPLAY_APOLLO
	aal_data->primary_data->hist.apollo_backlight = oplus_display_brightness;
#endif
...
#define CURVE_AEE_STR "AAL non-linear %s, %d. x_num %d, blk(%d, %d), offset %u expect 0x%x but 0x%x TS: 0x%08llx\n"
...
int mtk_drm_ioctl_aal_set_trigger_state_impl(struct mtk_ddp_comp *comp, void *data)
{
	...
	DDPMSG("%s dre3EnState: 0x%x, trigger_state: %d, ali: %d, aliThres: %d\n",
			__func__, dre3EnState,
...
#ifdef OPLUS_FEATURE_DISPLAY_APOLLO
	aal_data->primary_data->hist.apollo_backlight = -1;
#endif
	aal_data->primary_data->hist.essStrengthIndex = ESS_LEVEL_BY_CUSTOM_LIB;
#ifdef OPLUS_FEATURE_DISPLAY
	aal_data->primary_data->hist.ess_enable = ESS_OFF;
#else
...
#ifdef OPLUS_FEATURE_DISPLAY
	aal_data->primary_data->ess_en = ESS_OFF;
#else
