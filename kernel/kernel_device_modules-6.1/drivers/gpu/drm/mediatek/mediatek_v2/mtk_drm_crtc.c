...
extern bool g_is_mode_switch_pack;
extern atomic_t oplus_pcp_handle_lock;
extern wait_queue_head_t oplus_pcp_lock_clear_wq;
extern int oplus_pcp_lock_wait_clear_thread(void *data);
extern struct task_struct *oplus_pcp_task;
#endif
...
extern void oplus_sync_panel_brightness_video(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle);
...
#ifdef OPLUS_FEATURE_DISPLAY
extern bool panel_is_dunhuang(void);
...
extern int oplus_mtk_drm_setbacklight(struct drm_crtc *crtc, unsigned int level);
extern int oplus_mtk_dc_backlight_exit(struct drm_crtc *crtc);
extern int oplus_panel_ext_init(struct drm_crtc *crtc);
...
mtk_drm_crtc_duplicate_state(struct drm_crtc *crtc)
{
	...
	if (state->base.crtc != crtc)
		DDPPR_ERR("%s:%d, invalid crtc:(%p,%p)\n",
			__func__, __LINE__,
			state->base.crtc, crtc);
	state->base.crtc = crtc;
				...
				struct cmdq_pkt *cmdq_handle)
{
	...
	if (((oplus_dc_enable != oplus_dc_enable_real) && (crtc_id == 0)) || oplus_dc_recovery) {
...
int drm_mode_mm_clk (const struct drm_display_mode *mode)
{
	u32 mm_clk_src;


	mm_clk_src = mode->hdisplay * mode->vtotal * drm_mode_vrefresh(mode);
	pr_notice("[vivo] hac=%d vtotal=%d mm_clk=%d \n",
	mode->hdisplay, mode->vtotal, mm_clk_src);
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	if (output_comp && drm_mode_mm_clk(&crtc->state->mode) >=
		drm_mode_mm_clk(&old_state->mode) &&
		!mtk_crtc_is_frame_trigger_mode(crtc)) {
		if (mtk_crtc->qos_ctx) {
			mtk_crtc->qos_ctx->last_mmclk_req_idx += 1;
...
#endif
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_RPO) &&
		...
		if (output_comp) {
#ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT
			if (oplus_ofp_video_mode_30hz_aod_is_enabled() && fps_dst == 30)
...
#ifdef OPLUS_FEATURE_DISPLAY
	DDPINFO("%s: cur_fps:%d, skip_merge_trigger_offset = %u, merge_trigger_offset = %u, prefetch_te_offset = %u, frame_time = %u\n",
			__func__, cur_fps, skip_merge_trigger_offset, merge_trigger_offset, prefetch_te_offset, frame_time);
#endif /* OPLUS_FEATURE_DISPLAY */
...
#endif
{
	...
	if (get) {
		...
		atomic_inc(&mtk_crtc->wk_lock_ref);
	} else {
		__pm_relax(mtk_crtc->wk_lock);
		atomic_dec(&mtk_crtc->wk_lock_ref);
	}
	DDPMSG("CRTC%d %s wakelock(ref %d) %s %d\n",
		drm_crtc_index(crtc), (get ? "hold" : "release"),
		atomic_read(&mtk_crtc->wk_lock_ref) - 1, func, line);
}
	...
	void *cb_data, struct cmdq_pkt *cmdq_handle)
{
	...
	if (!(oplus_apollo_unsupported())) {
		if(mtk_crtc_is_frame_trigger_mode(crtc)) {
			...
			oplus_sync_panel_brightness_video(crtc, cmdq_handle);
		}
	}
#endif /* OPLUS_FEATURE_DISPLAY_APOLLO */
#ifdef MTK_DRM_CMDQ_ASYNC
				      ...
				      struct drm_atomic_state *atomic_state)
{
...
#if defined(CONFIG_PXLW_IRIS)
		if (iris_is_chip_supported() && comp
			&& (comp->id == DDP_COMPONENT_DSI0 || comp->id == DDP_COMPONENT_DSI1))
			iris_drm_kickoff(comp->id == DDP_COMPONENT_DSI1);
#endif
		if (mtk_crtc->is_dual_pipe) {
			for_each_comp_in_dual_pipe(comp, mtk_crtc, i, j) {
				if (crtc->state->color_mgmt_changed)
	...
	if (index == 2) {
		...
		DDPINFO("%s, current_time = %llu, oplus_last_te_time = %llu\n"
			, __func__, current_time, oplus_last_te_time);
		if ((current_time - oplus_last_te_time) / 1000 > 16000) {
			usleep_range(1000, 1100);
			drm_trace_tag_mark("atomic_flush sleep");
		}
	}
#endif /* OPLUS_FEATURE_DISPLAY */
...
#define CAPS_CHAR_SIZE 25
	...
	mtk_crtc->sf_pf_release_thread =
				kthread_run(mtk_drm_sf_pf_release_thread,
					    mtk_crtc, "sf_pf_release_thread");
	atomic_set(&mtk_crtc->force_high_step, 0);
	atomic_set(&mtk_crtc->wk_lock_ref, 0);
...
#ifdef OPLUS_FEATURE_DISPLAY
	oplus_pcp_task = kthread_create(
	oplus_pcp_lock_wait_clear_thread, NULL, "oplus_pcp_lock_clear_thread");
	cpumask_xor(&cpumask, cpu_all_mask, cpumask_of(0));
	kthread_bind_mask(oplus_pcp_task, &cpumask);
	atomic_set(&oplus_pcp_handle_lock, 0);
	init_waitqueue_head(&oplus_pcp_lock_clear_wq);
	wake_up_process(oplus_pcp_task);
#endif
...
int oplus_mtk_drm_setcabc(struct drm_crtc *crtc, unsigned int cabc_mode)
{
	...
	if (!dsi->ext || !dsi->ext->funcs || !dsi->ext->funcs->cabc_switch) {
		DDPMSG("%s invalid cabc_switch funcs\n", __func__);
		goto done;
	}
...
EXPORT_SYMBOL(oplus_mtk_drm_setcabc);
