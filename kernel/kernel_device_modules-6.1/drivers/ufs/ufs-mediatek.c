...
static void ufs_mtk_fix_ufs4_downgrade(struct ufs_hba *hba);
...
static bool ufs_mtk_is_ufs4_downgrade(struct ufs_hba *hba)
{
	struct ufs_mtk_host *host = ufshcd_get_variant(hba);

	return !!(host->caps & UFS_MTK_CAP_UFS4_DOWNGRADE_UFS31);
}
...
static void ufs_mtk_fix_ufs4_downgrade(struct ufs_hba *hba)
{
	...
	host->caps |= UFS_MTK_CAP_UFS4_DOWNGRADE_UFS31;
}
...
static void ufs_mtk_fix_clock_scaling(struct ufs_hba *hba)
{
	...
	if ((hba->dev_info.wspecversion < 0x0400 || ufs_mtk_is_ufs4_downgrade(hba)) &&
				  ...
				  struct ufs_pa_layer_attr *dev_req_params)
{
	...
	if(ufs_mtk_is_ufs4_downgrade(hba)) {
...
static void ufs_mtk_fixup_dev_quirks(struct ufs_hba *hba)
{
	...
	ufs_mtk_fix_ufs4_downgrade(hba);
