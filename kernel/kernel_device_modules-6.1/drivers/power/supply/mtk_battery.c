...
int battery_type_check(int *battery_type)
{
	...
	if (batt_id == NULL) {
		bm_debug("[battery_type_check]: batt_id iio channel is null []\n");
...
int fgauge_get_profile_id(struct mtk_battery *gm)
{
	...
	if (external_authenticate_support) {
		...
		if (battery_id >= 0) {
			bm_err("%s: get battid suc from comline, battid %d\n", __func__, battery_id);
			return battery_id;
		}
...
int adc_battemp(struct mtk_battery *gm, int res)
{
	...
	if (use_more_precise_NTC_table) {
		...
		__func__, res1, tmp1, res2, tmp2, res, tbatt_value);
	} else {
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
        bm_err(" %s, fg_meter_resistance:%d, com_fg_meter_resistance:%d\n",
		__func__, fg_cust_data->fg_meter_resistance,
		fg_cust_data->com_fg_meter_resistance);
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	bm_err(" %s, com_r_fg_value:%d, r_fg_value:%d\n",
		__func__, fg_cust_data->com_r_fg_value,
		fg_cust_data->r_fg_value);
#endif
...
static bool meter_fg_30_get_gauge_hmac(void)
{
	int battery_id = 0;
	if (!oplus_gm || !oplus_gm->check_hmac_with_battery_id)
		return true;
...
/*add for distinguish fuelgague and outlay-gague*/
	...
	fg_read_dts_val(pdev->dev.of_node, "oplus,check_hmac_with_battery_id", &(gm->check_hmac_with_battery_id), 1);
	bm_err("%s, check_hmac_with_battery_id:%d\n", __func__, gm->check_hmac_with_battery_id);
	fg_read_dts_val(pdev->dev.of_node, "oplus_spec,removed_bat_decidegc", &(gm->removed_bat_decidegc), 1);
	gm->removed_bat_decidegc = -gm->removed_bat_decidegc;
