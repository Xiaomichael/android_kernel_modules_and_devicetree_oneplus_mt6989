...
struct mt6375_chg_data {
	...
	struct mutex detach_count_lock;
#endif
					  ...
					  int attach)
{
	...
	if (attach != 0)
		detach_count = 0;
	mutex_unlock(&ddata->detach_count_lock);
#endif
...
static int mt6375_enable_buck(struct charger_device *chgdev, bool en)
{
	struct mt6375_chg_data *ddata = charger_get_data(chgdev);
/*#ifdef OPLUS_FEATURE_CHG_BASIC
	//?need to fix int boot_mode = get_boot_mode();
	int boot_mode = 0;
#endif*/
...
/*#ifdef OPLUS_FEATURE_CHG_BASIC
	if (boot_mode == META_BOOT || boot_mode == FACTORY_BOOT ||
		...
		mt_dbg(ddata->dev, "Meta mode or FTM stop charging!\n");
		return mt6375_chg_field_set(ddata, F_BUCK_EN, 0);
	}
#endif*/
...
#ifdef CONFIG_OPLUS_HVDCP_SUPPORT
	mutex_init(&ddata->detach_count_lock);
#endif
...
#ifdef CONFIG_OPLUS_HVDCP_SUPPORT
	mutex_destroy(&ddata->detach_count_lock);
#endif
