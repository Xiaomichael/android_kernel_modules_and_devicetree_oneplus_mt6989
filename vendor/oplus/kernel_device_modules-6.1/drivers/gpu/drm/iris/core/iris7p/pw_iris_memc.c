...
static void iris_dtg_setting_update(void *handle, bool lock, bool i2c_path)
{
	...
	switch (pcfg->ap_te) {
		case 144:
			...
			cmd[idx++] = 0x1802;	// VFP_MAX
			cmd[idx++] = IRIS_DTG_ADDR + DTG_VFP_MAX_RFB;
			cmd[idx++] = 0x1802;	// VFP_MAX_RFB
		...
		case 120:
			...
			cmd[idx++] = IRIS_DTG_ADDR + DTG_IVSA_FILTER_CTRL;
			cmd[idx++] = 0x0000095A;//payload[9];
			cmd[idx++] = IRIS_DTG_ADDR + DTG_TE_FILTER_CTRL;
			cmd[idx++] = 0x0000095A;//payload[10];
			cmd[idx++] = IRIS_DTG_ADDR + DTG_FI_PRELOAD_DLY;
			cmd[idx++] = 0x00000952;//payload[13];
			cmd[idx++] = IRIS_DTG_ADDR + DTG_EVS_PRE_DLY;
			cmd[idx++] = 0x00000952;//payload[16];
			cmd[idx++] = IRIS_DTG_ADDR + DTG_VFP_MIN;
			...
			cmd[idx++] = 0x2;
			if (!i2c_path) {
				cmd[idx++] = pcfg->iris_mipi_tx_header_addr;
				cmd[idx++] = 0x5000239;
				cmd[idx++] = pcfg->iris_mipi_tx_payload_addr;
				cmd[idx++] = 0x002F;
			}
	...
	} else if (handle && pcfg->lightup_ops.vdo_mode_send_cmd_with_handle) {
		/* dx-1 handle need to be zero*/
		pcfg->lightup_ops.vdo_mode_send_cmd_with_handle(handle, cmd, (idx << 2), 1, type);
		...
		if (pcfg->abyp_ctrl.abypass_mode != PASS_THROUGH_MODE) {
			...
			if (pcfg->pwil_mode == PT_MODE) {
				...
				if (new_te != pcfg->panel_te) {
					pcfg->panel_te = new_te;
					pcfg->ap_te = new_te;
					iris_dtg_setting_update(handle, true, false);
