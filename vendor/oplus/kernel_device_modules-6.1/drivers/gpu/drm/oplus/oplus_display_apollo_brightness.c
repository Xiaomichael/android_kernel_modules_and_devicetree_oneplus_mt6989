...
void oplus_sync_panel_brightness_video(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	if (!mtk_crtc->oplus_apollo_br) {
		mtk_crtc->oplus_apollo_br = kzalloc(sizeof(struct oplus_apollo_brightness),
			GFP_KERNEL);
		DDPPR_ERR("%s: oplus_apollo_brightness need allocate memory\n", __func__);
	}
	...
	oplus_disp_trace_begin("sync_panel_brightness_video level(%d) sync(%d)", mtk_crtc->oplus_apollo_br->oplus_pending_backlight,
							mtk_crtc->oplus_apollo_br->oplus_backlight_need_sync);
	...
	oplus_disp_trace_end("sync_panel_brightness_video level(%d) sync(%d)", mtk_crtc->oplus_apollo_br->oplus_pending_backlight,
							mtk_crtc->oplus_apollo_br->oplus_backlight_need_sync);
}
	...
	unsigned int panel_ext_param, unsigned int cfg_flag)
{
	...
	if ((mtk_crtc->oplus_apollo_br->oplus_backlight_need_sync || adfr_fps_need_sync)
	&& mtk_crtc->oplus_apollo_br->cur_vsync != 0
	&& oplus_bl_demura_dbv_switched == 0) {
