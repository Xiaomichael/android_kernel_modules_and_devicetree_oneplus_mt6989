...
#include "include/aa600_data_hw_roundedpattern.h"
#include "aa600_p_7_a0025_vdo_panel.h"
#include "../../oplus/oplus_drm_disp_panel.h"
...
static int get_mode_enum(struct drm_display_mode *m)
{
	...
	if (m == NULL) {
		DISP_ERR("aa600_p_7_a0025 %s display_mode m = NULL!\n", __func__);
		return -EINVAL;
	}
...
{
	DISP_ERR("aa600_p_7_a0025 %s +, mode id=%d\n", __func__, mode_id);
	switch (mode_id) {
	case FHD_SDC60:
	...
	DISP_ERR("%s,aa600_p_7_a0025 restore seed_mode:%d\n", __func__, temp_seed_mode);
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	DISP_ERR("aa600_p_7_a0025 backlight = %d bl_level[1]=%x, bl_level[2]=%x\n", level, bl_level[1], bl_level[2]);
	oplus_display_brightness = level;
	lcdinfo_notify(LCM_BRIGHTNESS_TYPE, &level);

	if (mapped_level <= 1153 && demura_tap != 1) {
		demura_tap = 1;
		DISP_ERR("aa600_p_7_a0025 backlight send demura1\n");
		...
		demura_tap = 2;
		DISP_ERR("aa600_p_7_a0025 backlight send demura2\n");
		...
		demura_tap = 3;
		DISP_ERR("aa600_p_7_a0025 backlight send demura3\n");
	...
	DISP_ERR("aa600_p_7_a0025 setbacklight finish\n");
	return 0;
}
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	if (level <= 1153 && demura_tap != 1) {
		demura_tap = 1;
		DISP_ERR("aa600_p_7_a0025 esd send demura1\n");
		for (i = 0; i < sizeof(lcm_set_demura_offset1)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_set_demura_offset1[i].para_list, lcm_set_demura_offset1[i].count);
		}
	} else if(level > 1153 && level <= 4094 && demura_tap != 2) {
		demura_tap = 2;
		DISP_ERR("aa600_p_7_a0025 esd send demura2\n");
		for (i = 0; i < sizeof(lcm_set_demura_offset2)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_set_demura_offset2[i].para_list, lcm_set_demura_offset2[i].count);
		}
	} else if(level == 4095 && demura_tap != 3) {
		demura_tap = 3;
		DISP_ERR("aa600_p_7_a0025 esd send demura3\n");
		for (i = 0; i < sizeof(lcm_set_demura_offset3)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_set_demura_offset3[i].para_list, lcm_set_demura_offset3[i].count);
		}
	}

	DISP_ERR("aa600_p_7_a0025 esd_backlight_recovery finish\n");
	return 0;
}
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	DISP_ERR("%s: aa600_p_7_a0025 poweron Start\n", __func__);
	//iovcc enable 1.8V
	...
	DISP_ERR("%s: aa600_p_7_a0025 poweron Successful\n", __func__);
	return 0;
}
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	DISP_ERR("%s: aa600_p_7_a0025 lcm ctx->prepared %d\n", __func__, ctx->prepared);
	...
	if (ret < 0)
		lcm_unprepare(panel);
	demura_tap = 0;
	DISP_ERR("%s:aa600_p_7_a0025 Successful\n", __func__);
		...
		unsigned int id)
{
	...
	if (*ext_param)
		DISP_DEBUG("aa600_p_7_a0025 data_rate:%d\n", (*ext_param)->data_rate);
	else
		DISP_ERR("aa600_p_7_a0025 ext_param is NULL;\n");
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("[LCM] aa600_p_7_a0025 %s START\n", __func__);
	...
	pr_info("[LCM] %s- lcm, aa600_p_7_a0025, END\n", __func__);
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "aa600_p_7_a0025_vdo_panel",
		.owner = THIS_MODULE,
