...
#include "../mtk_round_corner/data_hw_roundedpattern_ab948.h"
#endif
#include "panel_ab948_p_7_a0020_dsi_cmd.h"
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (oplus_panel_pwm_onepulse_is_enabled()) {
		LCM_INFO("Send pwm_switch_3to1 when panel-init\n");
		switch (mode_id) {
		case FHD_SDC60:
			LCM_INFO("fhd_dsi_on_cmd_sdc60\n");
			push_table(ctx, dsi_on_cmd_sdc60_1pulse, sizeof(dsi_on_cmd_sdc60_1pulse) / sizeof(struct LCM_setting_table));
			break;
		case FHD_SDC90:
			LCM_INFO("fhd_dsi_on_cmd_sdc90\n");
			push_table(ctx, dsi_on_cmd_sdc90_1pulse, sizeof(dsi_on_cmd_sdc90_1pulse) / sizeof(struct LCM_setting_table));
			break;
		case FHD_SDC120:
			LCM_INFO("fhd_dsi_on_cmd_sdc120\n");
			push_table(ctx, dsi_on_cmd_sdc120_1pulse, sizeof(dsi_on_cmd_sdc120_1pulse) / sizeof(struct LCM_setting_table));
			break;
		case FHD_OPLUS120:
			LCM_INFO("fhd_dsi_on_cmd_oplus120\n");
			push_table(ctx, dsi_on_cmd_oa120_1pulse, sizeof(dsi_on_cmd_oa120_1pulse) / sizeof(struct LCM_setting_table));
			break;
		default:
...
void oplus_display_panel_pwm_switch_prepare(unsigned int level) {
	...
	if (!oplus_panel_pwm_onepulse_is_enabled() && (last_backlight > 10) && (level > 10)) {
		if (condition1) {
			...
			LCM_INFO("Send pwm_switch_18to3\n");
		} else if (condition2) {
			pulse_flg = true;
	...
	switch (mode) {
		case PWM_SWITCH_18TO3: {
			...
			if (panel_mode_id == FHD_SDC120 || panel_mode_id == FHD_OPLUS120) {
				...
				pr_info("pwm_turbo dsi_pwm_switch 120HZ 18to3\n");
				...
				pr_info("pwm_turbo dsi_pwm_switch 90HZ 18to3\n");
				...
				pr_info("pwm_turbo dsi_pwm_switch 60HZ 18to3\n");
			}
		}
		break;
		...
		case PWM_SWITCH_1TO3: {
			...
			if (panel_mode_id == FHD_SDC120 || panel_mode_id == FHD_OPLUS120) {
				...
				panel_send_pack_hs_cmd(dsi, dsi_pwm_switch_pwm_compensate_120hz,
					sizeof(dsi_pwm_switch_pwm_compensate_120hz) /sizeof(struct LCM_setting_table), cb, handle);
			} else if (panel_mode_id == FHD_SDC90) {
				panel_send_pack_hs_cmd(dsi, dsi_pwm_switch_1to3pul_90hz, sizeof(dsi_pwm_switch_1to3pul_90hz) /
					sizeof(struct LCM_setting_table), cb, handle);

				panel_send_pack_hs_cmd(dsi, dsi_pwm_switch_pwm_compensate_90hz,
					sizeof(dsi_pwm_switch_pwm_compensate_90hz) /sizeof(struct LCM_setting_table), cb, handle);
			} else if (panel_mode_id == FHD_SDC60) {
		...
		case PWM_SWITCH_3TO1: {
			...
			if (panel_mode_id == FHD_SDC120 || panel_mode_id == FHD_OPLUS120) {
				...
				panel_send_pack_hs_cmd(dsi, dsi_pwm_switch_dc_compensate_120hz,
					sizeof(dsi_pwm_switch_dc_compensate_120hz) /sizeof(struct LCM_setting_table), cb, handle);

				pr_info("pwm_turbo onepulse dsi_pwm_switch 120hz 3to1\n");
			} else if (panel_mode_id == FHD_SDC90) {
				panel_send_pack_hs_cmd(dsi, dsi_pwm_switch_3to1pul_90hz, sizeof(dsi_pwm_switch_3to1pul_90hz) /
					sizeof(struct LCM_setting_table), cb, handle);

				panel_send_pack_hs_cmd(dsi, dsi_pwm_switch_dc_compensate_90hz,
					sizeof(dsi_pwm_switch_dc_compensate_90hz) /sizeof(struct LCM_setting_table), cb, handle);

				pr_info("pwm_turbo onepulse dsi_pwm_switch 90hz 3to1\n");
			} else if (panel_mode_id == FHD_SDC60) {
				...
				pr_info("pwm_turbo onepulse dsi_pwm_switch 60hz 3to1\n");
			}
		}
		break;
					...
					struct drm_connector *connector)
{
	...
	connector->display_info.height_mm = PHYSICAL_HEIGHT / 1000;
  	...
  	if (!ctx->m) {
  		...
  		LCM_INFO("lvqiangjian  ctx->m init: mode_id %d\n",get_mode_enum(ctx->m));
  	}
