...
typedef enum {
    SGM38120_PRODUCT_ID = 0x00,
    SGM38120_SILICON_REV,
    SGM38120_IOUT,
    SGM38120_ENABLE,
    SGM38120_LDO1VOUT,
    SGM38120_LDO2VOUT,
    ...
    SGM38120_LDO6VOUT,
    SGM38120_LDO7VOUT,
    SGM38120_REG_MAX = 0x1F,
} multi_reg_sgm38120_registers_t;
...
#define SGM38120_NLDO(_num, _supply, _default)                \
    [SGM38120_LDO ## _num] = {                    \
        .name =           "ONLDO"#_num,            \
        .of_match =       of_match_ptr("ONLDO"#_num),        \
        .regulators_node = of_match_ptr("regulators"),        \
        ...
        .owner =       THIS_MODULE,                \
        .linear_ranges =   (struct linear_range[]) {        \
              REGULATOR_LINEAR_RANGE(_default, 0x00, 0x02, 0),    \
              REGULATOR_LINEAR_RANGE(_default, 0x03, 0x7D, 8000),    \
        },                            \
        .n_linear_ranges = 2,                    \
        .vsel_reg =       SGM38120_LDO ## _num ## VOUT,    \
        .vsel_mask =       0xff,                \
        .enable_reg =       SGM38120_ENABLE,            \
        .enable_mask =       BIT(_num - 1),            \
        .enable_time =       150,                    \
        .supply_name =       _supply,                \
        .ops =           &multi_reg_ops,            \
...
#define SGM38120_PLDO(_num, _supply, _default)                \
    [SGM38120_LDO ## _num] = {                    \
        .name =           "ONLDO"#_num,                \
        .of_match =       of_match_ptr("ONLDO"#_num),        \
        .regulators_node = of_match_ptr("regulators"),        \
        ...
        .owner =       THIS_MODULE,                \
        .linear_ranges =   (struct linear_range[]) {        \
              REGULATOR_LINEAR_RANGE(_default, 0x00, 0xff, 8000),    \
        },                            \
        .n_linear_ranges = 1,                    \
        .vsel_reg =       SGM38120_LDO ## _num ## VOUT,    \
        .vsel_mask =       0xff,                \
...
static struct regulator_desc multi_reg_sgm38120_regulators[] = {
    SGM38120_NLDO(1, "vin12", 528000),
    SGM38120_NLDO(2, "vin12", 528000),
    SGM38120_PLDO(3, "vin34", 1504000),
    SGM38120_PLDO(4, "vin34", 1504000),
    SGM38120_PLDO(5, "vin5", 1504000),
    SGM38120_PLDO(6, "vin6", 1504000),
    SGM38120_PLDO(7, "vin7", 1504000),
};
...
static mult_reg_dev_info_t dev_info[] = {
    ...
    /*sgm38120*/
    {.mult_reg_slave_id = 0x35,
     .product_id_reg = SGM38120_PRODUCT_ID,
     .product_id = 0xD9,
     .reg_desc = &multi_reg_sgm38120_regulators[0],
     .reg_desc_size = ARRAY_SIZE(multi_reg_sgm38120_regulators),
    },
    /*Ivalid*/
    {0xff},
};
                ...
                const struct i2c_device_id *id)
{
    ...
    /*wangjie@camera add for iwc delay 1ms after reset high*/
    if (is_project(25612) || is_project(25693) || is_project(25694)) {
        mdelay(1);
    }
    ...
    while(dev_info[index].mult_reg_slave_id != 0xFF){
        ...
        /*wangjie@camera Avoid falling into an infinite loop when the dev_info array is greater than 2*/
        access_time = 3;
    ...
    dev_info(&i2c->dev, "find the regulator ic, slaveId:0x%x,product_id=0x%x\n", dev_info[index].mult_reg_slave_id,data);
