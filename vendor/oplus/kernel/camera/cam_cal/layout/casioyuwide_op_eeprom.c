...
#define PFX "CAM_CAL_CASIOYUWIDE"
...
static unsigned int do_single_lsc_casioyuwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_casioyuwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_casioyuwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static unsigned int layout_check_casioyuwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x016B012B, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_casioyuwide},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_casioyuwide},  //Start address, block size is useless
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_casioyuwide}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT casioyuwide_op_eeprom = {
	.name = "casioyuwide_op_eeprom",
	.check_layout_function = layout_check_casioyuwide,
...
static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &casioyuwide_op_eeprom;
...
static unsigned int layout_check_casioyuwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID)
{
	...
	if (check_id == 0x016B012B || check_id == 0x016d009a || check_id ==0x0065009a) {	// hearder id on OTP guide
		debug_log("header_id matched 0x%08x\n", check_id);
		printk("casioyheader_id wide matched 0x%08x\n", check_id);
		result = CAM_CAL_ERR_NO_ERR;
	} else{
		debug_log("header_id not matched 0x%08x\n", check_id);
		printk("casioyheader_id wide not matched 0x%08x\n", check_id);
	}
	return result;
}
