...
#define PFX "CAM_CAL_CASIOYFRONT2"
...
static unsigned int do_single_lsc_casioyfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_casioyfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_casioyfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_casioyfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x04010115, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000600, 0x0000074C, do_single_lsc_casioyfront2},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_casioyfront2}, //Start address, block size is useless
		{0x00000001, 0x00001300, 0x000001F0, do_pdaf_casioyfront2},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00004000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_casioyfront2}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT casioyfront2_op_eeprom = {
	.name = "casioyfront2_op_eeprom",
...
#define CASIOYFRONT2_HVBIN_PDAF_PROC1_SIZE  (0x14F0-0x1300) //496      0x1F0
#define CASIOYFRONT2_HVBIN_PDAF_PROC2_SIZE  (0x18EC-0x1500) //1004     0X3EC
#define CASIOYFRONT2_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define CASIOYFRONT2_HVBIN_PDAF_PROC2_ADDR  (0x1500)

//partial PD
/*
#define CASIOYFRONT2_PARTIAL_PD_PROC1_SIZE  (0x3590-0x33A0) //496       0X1F0
#define CASIOYFRONT2_PARTIAL_PD_PROC2_SIZE  (0x398C-0x35A0) //1004      1500   0x5DC
#define CASIOYFRONT2_PARTIAL_PD_PROC1_ADDR  (0x33A0)
#define CASIOYFRONT2_PARTIAL_PD_PROC2_ADDR  (0x35A0)
*/
unsigned int do_pdaf_casioyfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	debug_log("======================PDAF Data==================\n");
	debug_log("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOYFRONT2_HVBIN_PDAF_PROC1_ADDR, CASIOYFRONT2_HVBIN_PDAF_PROC1_SIZE);
	debug_log("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOYFRONT2_HVBIN_PDAF_PROC2_ADDR, CASIOYFRONT2_HVBIN_PDAF_PROC2_SIZE);
	//debug_log("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOYFRONT2_PARTIAL_PD_PROC1_ADDR, CASIOYFRONT2_PARTIAL_PD_PROC1_SIZE);
	//debug_log("[%s] partial PD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOYFRONT2_PARTIAL_PD_PROC2_ADDR, CASIOYFRONT2_PARTIAL_PD_PROC2_SIZE);

// QPD
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			CASIOYFRONT2_HVBIN_PDAF_PROC1_ADDR + CASIOYFRONT2_HVBIN_PDAF_PROC1_SIZE,
	...
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			...
			bios += CASIOYFRONT2_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CASIOYFRONT2_HVBIN_PDAF_PROC2_ADDR + CASIOYFRONT2_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += CASIOYFRONT2_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
