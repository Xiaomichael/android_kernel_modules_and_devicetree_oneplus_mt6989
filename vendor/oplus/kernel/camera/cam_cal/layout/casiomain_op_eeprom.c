...
static unsigned int do_single_lsc_casiomain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_casiomain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_casiomain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_casiomain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static unsigned int layout_check_casiomain(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x016B012B, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00001070, 0x0000074C, do_single_lsc_casiomain},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_casiomain}, //Start address, block size is useless
		{0x00000001, 0x000017D0, 0x00000A3A, do_pdaf_casiomain},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_casiomain}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT casiomain_op_eeprom = {
	.name = "casiomain_op_eeprom",
	.check_layout_function = layout_check_casiomain,
	.read_function = Common_read_region,
	.layout = &cal_layout_table,
	.sensor_id = CASIOMAIN_SENSOR_ID,
	.i2c_write_id = 0xA0,
	.max_size = 0x8000,
	.enable_preload = 1,
	.preload_size = 0x8000,
...
static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &casiomain_op_eeprom;

/*
unsigned int do_pdaf_casiomain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#define CASIOMAIN_HVBIN_PDAF_PROC1_SIZE  (0x1BA0-0x17D0) //976      0x3D0
#define CASIOMAIN_HVBIN_PDAF_PROC2_SIZE  (0x2208-0x1BB0) //1624     2600  0xA28
#define CASIOMAIN_HVBIN_PDAF_PROC1_ADDR  (0x17D0)
#define CASIOMAIN_HVBIN_PDAF_PROC2_ADDR  (0x1BB0)

//partial PD
#define CASIOMAIN_PARTIAL_PD_PROC1_SIZE  (0x3590-0x33A0) //496       0X1F0
#define CASIOMAIN_PARTIAL_PD_PROC2_SIZE  (0x398C-0x35A0) //1004      1500   0x5DC
#define CASIOMAIN_PARTIAL_PD_PROC1_ADDR  (0x33A0)
#define CASIOMAIN_PARTIAL_PD_PROC2_ADDR  (0x35A0)
unsigned int do_pdaf_casiomain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
	...
	debug_log("======================PDAF Data==================\n");
	debug_log("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOMAIN_HVBIN_PDAF_PROC1_ADDR, CASIOMAIN_HVBIN_PDAF_PROC1_SIZE);
	debug_log("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOMAIN_HVBIN_PDAF_PROC2_ADDR, CASIOMAIN_HVBIN_PDAF_PROC2_SIZE);
	debug_log("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOMAIN_PARTIAL_PD_PROC1_ADDR, CASIOMAIN_PARTIAL_PD_PROC1_SIZE);
	debug_log("[%s] partial PD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CASIOMAIN_PARTIAL_PD_PROC2_ADDR, CASIOMAIN_PARTIAL_PD_PROC2_SIZE);
...
// partial PD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			CASIOMAIN_PARTIAL_PD_PROC1_ADDR + CASIOMAIN_PARTIAL_PD_PROC1_SIZE,
	...
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			...
			bios += CASIOMAIN_PARTIAL_PD_PROC1_SIZE;
	...
	// partial PD proc2
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CASIOMAIN_PARTIAL_PD_PROC2_ADDR + CASIOMAIN_PARTIAL_PD_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			...
			if (read_data_size > 0) {
				debug_log("[%s] partial PD proc2 First five %x, %x, %x, %x, %x\n",
					...
					pCamCalData->PDAF.Data[bios + 3],
					pCamCalData->PDAF.Data[bios + 4]);
				bios += CASIOMAIN_PARTIAL_PD_PROC2_SIZE;
...
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			CASIOMAIN_HVBIN_PDAF_PROC1_ADDR + CASIOMAIN_HVBIN_PDAF_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc1 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += CASIOMAIN_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CASIOMAIN_HVBIN_PDAF_PROC2_ADDR + CASIOMAIN_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += CASIOMAIN_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
