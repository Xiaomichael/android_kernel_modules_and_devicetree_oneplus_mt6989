...
static struct sensor_info support_sensors1[] = {
	{
		.sensor_type = SENSOR_TYPE_OIS_POWER,
		.gain = 1,
		.name = "ois1_power",
		.vendor = "oplus"
	},
...
{
	int ret;
	printk("oisdebug ois_power_open\n");
...
{
	printk("oisdebug ois_power_close\n");
...
static int ois1_power_enable(struct hf_device *hfdev, int sensor_type, int en)
{
	...
	int ret1 = 0;
	int prj_id = 0;
	printk("oisdebug:ois1_power_enable\n");
	if (0 == hfdev) {
	...
	if (en) {
		...
		if ((24749 == prj_id) || (24622 == prj_id)) {
			ret1 = regulator_set_voltage(ois_power_dev->vin, 3300000, 3300000);
		} else {
...
static int ois_power_init_controls(struct ois_power_device *ois_power_dev)
{
	...
	printk("oisdebug ois_power_init_controls\n");
...
static int ois_power_probe(struct i2c_client *client)
{
	...
	LOG_INF("E");
	printk("oisdebug:ois_power_probe\n");
