 ...
 *============================================================================
 ****************************************************************************/
#include "zhuquec2telemipiraw_Sensor.h"

#define SENSOR_NAME  SENSOR_DRVNAME_ZHUQUEC2TELE_MIPI_RAW
#define PFX "zhuquec2tele_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)
#define ZHUQUEC2TELE_EEPROM_READ_ID     (0xA1)
#define ZHUQUEC2TELE_EEPROM_WRITE_ID    (0xA0)

#define OTP_SIZE    (0x8000)
#define ZHUQUEC2TELE_UNIQUE_SENSOR_ID_ADDR 0x0A24
#define ZHUQUEC2TELE_UNIQUE_SENSOR_ID_LENGTH 6

static BYTE zhuquec2tele_unique_id[ZHUQUEC2TELE_UNIQUE_SENSOR_ID_LENGTH] = { 0 };
#ifdef  EEPROM_WRITE_DATA_MAX_LENGTH
#undef  EEPROM_WRITE_DATA_MAX_LENGTH
#endif
#define EEPROM_WRITE_DATA_MAX_LENGTH      (64)
#define ZHUQUEC2TELE_STEREO_MT_START_ADDR  (0x22C0)
#define ZHUQUEC2TELE_STEREO_MT105_START_ADDR  (0x2640)
#define ZHUQUEC2TELE_AESYNC_START_ADDR     (0x2290)
#define ZHUQUEC2TELE_EEPROM_LOCK_REGISTER  (0xA000)
...
static int zhuquec2tele_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_get_eeprom_comdata(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static void zhuquec2tele_set_gain_convert(struct subdrv_ctx *ctx, u32 gain);
static int zhuquec2tele_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void zhuquec2tele_set_multi_gain(struct subdrv_ctx *ctx, u32 *gains, u16 exp_cnt);
static void zhuquec2tele_set_hdr_tri_gain(struct subdrv_ctx *ctx, u64 *gains, u16 exp_cnt);
static int zhuquec2tele_set_hdr_tri_gain2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_hdr_tri_gain3(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static void zhuquec2tele_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter);
static int zhuquec2tele_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void zhuquec2tele_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static void zhuquec2tele_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length);
static int zhuquec2tele_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_video_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
// static int zhuquec2tele_set_sensor_rmsc_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_i2c_burst_wr_regs_u16(struct subdrv_ctx *ctx, u16 * list, u32 len);
static int adapter_i2c_burst_wr_regs_u16(struct subdrv_ctx * ctx,
		u16 addr, u16 *list, u32 len);
static void zhuquec2tele_lens_pos_writeback(struct subdrv_ctx *ctx);
static int zhuquec2tele_set_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_get_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static bool g_id_from_dts_flag = false;
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id);
static int zhuquec2tele_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_get_cloud_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_set_af_code_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_get_ois_compensation(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int zhuquec2tele_get_ois_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void zhuquec2tele_set_multi_shutter_frame_length_in_lut_convert(struct subdrv_ctx *ctx,
	u64 *shutters, u16 exp_cnt, u32 frame_length, u32 *frame_length_in_lut);
static int zhuquec2tele_set_multi_shutter_frame_length_in_lut(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void zhuquec2tele_write_frame_length_in_lut(struct subdrv_ctx *ctx, u32 fll, u32 *fll_in_lut);
static void zhuquec2tele_set_max_framerate_in_lut_by_scenario(struct subdrv_ctx *ctx,
	enum SENSOR_SCENARIO_ID_ENUM scenario_id, u32 framerate);
/* STRUCT */

// static struct oplus_get_eeprom_common_data zhuquec2tele_eeprom_common_data = {0};
static kal_uint16 g_af_code_macro    = 0;
static kal_uint16 g_af_code_infinity = 0;
// static kal_uint16 g_af_code_middle   = 0;

static struct eeprom_map_info zhuquec2tele_eeprom_info[] = {
	...
	{ EEPROM_META_STEREO_MT_MAIN_DATA, ZHUQUEC2TELE_STEREO_MT_START_ADDR, 0, 0, CALI_DATA_SLAVE_TELE_LENGTH, false },
	{ EEPROM_META_STEREO_MT_MAIN_DATA_105CM, ZHUQUEC2TELE_STEREO_MT105_START_ADDR, 0, 0, CALI_DATA_SLAVE_TELE_LENGTH, false },
};

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, zhuquec2tele_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, zhuquec2tele_set_test_pattern_data},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, zhuquec2tele_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, zhuquec2tele_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, zhuquec2tele_get_eeprom_comdata},
	{SENSOR_FEATURE_SET_SENSOR_OTP, zhuquec2tele_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, zhuquec2tele_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, zhuquec2tele_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, zhuquec2tele_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, zhuquec2tele_streaming_resume},
	{SENSOR_FEATURE_SET_GAIN, zhuquec2tele_set_gain},
	{SENSOR_FEATURE_SET_DUAL_GAIN, zhuquec2tele_set_hdr_tri_gain2},
	{SENSOR_FEATURE_SET_HDR_TRI_GAIN, zhuquec2tele_set_hdr_tri_gain3},
	{SENSOR_FEATURE_SET_ESHUTTER, zhuquec2tele_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, zhuquec2tele_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, zhuquec2tele_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, zhuquec2tele_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, zhuquec2tele_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, zhuquec2tele_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_SEAMLESS_EXTEND_FRAME_LENGTH, zhuquec2tele_extend_frame_length},
	{SENSOR_FEATURE_SET_VIDEO_MODE, zhuquec2tele_set_video_mode},
	// {SENSOR_FEATURE_SET_SENSOR_RMSC_MODE, zhuquec2tele_set_sensor_rmsc_mode},
	{SENSOR_FEATURE_SET_AWB_GAIN, zhuquec2tele_set_awb_gain},
	{SENSOR_FEATURE_SET_REGISTER, zhuquec2tele_set_register},
	{SENSOR_FEATURE_GET_REGISTER, zhuquec2tele_get_register},
	{SENSOR_FEATURE_GET_UNIQUE_SENSORID, zhuquec2tele_get_unique_sensorid},
	{SENSOR_FEATURE_GET_CLOUD_OTP_INFO, zhuquec2tele_get_cloud_otp_info},
	{SENSOR_FEATURE_SET_AF_CODE_DATA, zhuquec2tele_set_af_code_data},
	{SENSOR_FEATURE_GET_OIS_COMPENSATION, zhuquec2tele_get_ois_compensation},
	{SENSOR_FEATURE_GET_OIS_OTP_INFO, zhuquec2tele_get_ois_otp_info},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME_IN_LUT, zhuquec2tele_set_multi_shutter_frame_length_in_lut},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{/* mode0 4sum12.5Mp_30FPS_4096x3072*/
		...
		.mode_setting_table = zhuquec2tele_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_preview_capture_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = zhuquec2tele_bining_12_5M_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(zhuquec2tele_bining_12_5M_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{/* mode2 4sum4k_30FPS_4096x2304*/
		...
		.mode_setting_table = zhuquec2tele_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_normal_video_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = zhuquec2tele_bining_4k_30fps_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(zhuquec2tele_bining_4k_30fps_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{/* mode3 4sum4k_60FPS_4096x2304*/
		...
		.mode_setting_table = zhuquec2tele_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode4 4sum2bin_120FPS_2048x1152*/
		...
		.mode_setting_table = zhuquec2tele_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode5 4sum12.5Mp_24FPS_4096x3072*/
		...
		.mode_setting_table = zhuquec2tele_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode6 4sum2bin_24FPS_2048x1536*/
		...
		.mode_setting_table = zhuquec2tele_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode7 full50Mp_15FPS_8192x6144_remosaicON*/
		...
		.mode_setting_table = zhuquec2tele_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode8 4sum4k_30FPS_4096x2304_2expSHDR*/
		...
		.mode_setting_table = zhuquec2tele_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
	...
	{/* mode9 fullcrop12.5Mp_30FPS_4096x3072_remosaicON*/
		...
		.mode_setting_table = zhuquec2tele_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom5_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode10 4sum2bin_30FPS_2048x1152*/
		...
		.mode_setting_table = zhuquec2tele_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode11 4sum2bin_240FPS_2048x1152*/
		...
		.mode_setting_table = zhuquec2tele_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode12 4sumFHD_30FPS_2048x1152_2expSHDR*/
		...
		.mode_setting_table = zhuquec2tele_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_STAGGER,
	...
	{/* mode13 4sum2bin_30FPS_2048x1536*/
		...
		.mode_setting_table = zhuquec2tele_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode14 4sum2bin_15FPS_2048x1536*/
		...
		.mode_setting_table = zhuquec2tele_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode15 4sum2bin_15FPS_2048x1152*/
		...
		.mode_setting_table = zhuquec2tele_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode16 4sum4k_30FPS_3264x1836 */
		...
		.mode_setting_table = zhuquec2tele_custom12_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom12_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode17 4sum4k_15FPS_3264x1836 */
		...
		.mode_setting_table = zhuquec2tele_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom13_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode18 4sum12.5M_30FPS_4096x3072_2expSHDR*/
		...
		.mode_setting_table = zhuquec2tele_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom14_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_STAGGER,
	...
	{/* mode19 fullcrop12.5Mp_30FPS_4096x3072_remosaic0ff*/
		...
		.mode_setting_table = zhuquec2tele_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom15_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = zhuquec2tele_izoom_qbc_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(zhuquec2tele_izoom_qbc_seamless_setting),
	...
	{/* mode20 4sum2bin_30FPS_680x512*/
		...
		.mode_setting_table = zhuquec2tele_custom16_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom16_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode21 4sum4k_30FPS_3264x2448 */
		...
		.mode_setting_table = zhuquec2tele_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom17_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* mode22 video izoom*/
		...
		.mode_setting_table = zhuquec2tele_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom18_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = zhuquec2tele_izoom_4k_30fps_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(zhuquec2tele_izoom_4k_30fps_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{ /* 5_OceanZhuQue_JN5_Fdsum_AEB_4096x3072_60fps_3056Msps */
		.frame_desc = frame_desc_cus19,
		.num_entries = ARRAY_SIZE(frame_desc_cus19),
		.mode_setting_table = zhuquec2tele_custom19_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom19_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = zhuquec2tele_lbmf_12_5M_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(zhuquec2tele_lbmf_12_5M_seamless_setting),
	...
	{/* 05_1_OceanZhuQue_JN5_Fdsum_4096x2048_30fps_3056Msps*/
		...
		.mode_setting_table = zhuquec2tele_custom20_setting,
		.mode_setting_len = ARRAY_SIZE(zhuquec2tele_custom20_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = ZHUQUEC2TELE_SENSOR_ID,
	...
	.ana_gain_step = 2,
	.ana_gain_table = zhuquec2tele_ana_gain_table,
	.ana_gain_table_size = sizeof(zhuquec2tele_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
...
//	.init_setting_table = zhuquec2tele_sensor_init_setting,
//	.init_setting_len =  ARRAY_SIZE(zhuquec2tele_sensor_init_setting),
	.mode = mode_struct,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_DVDD, 1002000, 0},
	{HW_ID_AFVDD, 3300000, 0},
	{HW_ID_RST, 1, 2},
	{HW_ID_MCLK_DRIVING_CURRENT, 4, 10},
};

struct subdrv_entry zhuquec2tele_mipi_raw_entry = {
	.name = SENSOR_NAME,
	.id = ZHUQUEC2TELE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int zhuquec2tele_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			zhuquec2tele_set_shutter_convert(ctx, *(u32 *)para);
...
static unsigned int read_zhuquec2tele_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != zhuquec2tele_eeprom_info[meta_id].meta)
		return -1;

	if (size != zhuquec2tele_eeprom_info[meta_id].size)
		return -1;

	addr = zhuquec2tele_eeprom_info[meta_id].start;
	readsize = zhuquec2tele_eeprom_info[meta_id].size;
...
// 	memset(&zhuquec2tele_eeprom_common_data, 0x00, sizeof(zhuquec2tele_eeprom_common_data));

// 	zhuquec2tele_eeprom_common_data.header[EEPROM_MODULE_ID] = 2;
// 	read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_MODULE_ID,
// 				&(zhuquec2tele_eeprom_common_data.data[idx]), 2);
// 	idx += zhuquec2tele_eeprom_common_data.header[EEPROM_MODULE_ID];

// 	zhuquec2tele_eeprom_common_data.header[EEPROM_SENSOR_ID] = 2;
// 	read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_SENSOR_ID,
// 				&(zhuquec2tele_eeprom_common_data.data[idx]), 2);
// 	idx += zhuquec2tele_eeprom_common_data.header[EEPROM_SENSOR_ID];

// 	zhuquec2tele_eeprom_common_data.header[EEPROM_LENS_ID] = 2;
// 	read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_LENS_ID,
// 				&(zhuquec2tele_eeprom_common_data.data[idx]), 2);
// 	idx += zhuquec2tele_eeprom_common_data.header[EEPROM_LENS_ID];

// 	zhuquec2tele_eeprom_common_data.header[EEPROM_VCM_ID] = 2;
// 	read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_VCM_ID,
// 				&(zhuquec2tele_eeprom_common_data.data[idx]), 2);
// 	idx += zhuquec2tele_eeprom_common_data.header[EEPROM_VCM_ID];

// 	zhuquec2tele_eeprom_common_data.header[EEPROM_MODULE_SN] = 23;
// 	read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_MODULE_SN,
// 				&(zhuquec2tele_eeprom_common_data.data[idx]), 23);
// 	idx += zhuquec2tele_eeprom_common_data.header[EEPROM_MODULE_SN];

// 	zhuquec2tele_eeprom_common_data.header[EEPROM_AF_CODE_MACRO] = 2;
// 	zhuquec2tele_eeprom_common_data.header[EEPROM_AF_CODE_INFINITY] = 2;
// 	zhuquec2tele_eeprom_common_data.header[EEPROM_AF_CODE_MIDDLE] = 2;
// 	read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_AF_CODE,
// 				&(zhuquec2tele_eeprom_common_data.data[idx]), 6);

// 	AF_CODE_MACRO_idx = idx;
// 	AF_CODE_INFINITY_idx = idx + 2;
// 	AF_CODE_MIDDLE_idx = idx + 4;

// 	g_af_code_macro = zhuquec2tele_eeprom_common_data.data[AF_CODE_MACRO_idx + 1] << 8 |
// 		zhuquec2tele_eeprom_common_data.data[AF_CODE_MACRO_idx];
// 	g_af_code_infinity = zhuquec2tele_eeprom_common_data.data[AF_CODE_INFINITY_idx + 1] << 8 |
// 		zhuquec2tele_eeprom_common_data.data[AF_CODE_INFINITY_idx];;
// 	g_af_code_middle = zhuquec2tele_eeprom_common_data.data[AF_CODE_MIDDLE_idx + 1] << 8 |
// 		zhuquec2tele_eeprom_common_data.data[AF_CODE_MIDDLE_idx];;

// 	for (idx = 0; idx < 64; idx = idx + 4)
// 		LOG_INF("In %s:common data: %02x %02x %02x %02x\n", __func__,
// 			zhuquec2tele_eeprom_common_data.data[idx], zhuquec2tele_eeprom_common_data.data[idx + 1],
// 			zhuquec2tele_eeprom_common_data.data[idx + 2],
// 			zhuquec2tele_eeprom_common_data.data[idx + 3]);
// }
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, ZHUQUEC2TELE_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, ZHUQUEC2TELE_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
{
    kal_int32 ret = ERROR_NONE;
    kal_uint16 reg = ZHUQUEC2TELE_EEPROM_LOCK_REGISTER;
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, ZHUQUEC2TELE_EEPROM_WRITE_ID >> 1,
            reg, 0x0E);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, ZHUQUEC2TELE_EEPROM_WRITE_ID >> 1,
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == ZHUQUEC2TELE_SENSOR_ID || pStereodata->uSensorId == ZHUQUES2TELE_SENSOR_ID)
            && (data_length == CALI_DATA_SLAVE_TELE_LENGTH)
            && ((data_base == ZHUQUEC2TELE_STEREO_MT_START_ADDR) || (data_base == ZHUQUEC2TELE_STEREO_MT105_START_ADDR))) {
        ...
        } else if ((pStereodata->uSensorId == ZHUQUEC2TELE_SENSOR_ID || pStereodata->uSensorId == ZHUQUES2TELE_SENSOR_ID)
            && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == ZHUQUEC2TELE_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR),
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR+1),
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR+2),
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR+3),
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR+4),
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR+5),
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR+6),
                    read_cmos_eeprom_8(ctx, ZHUQUEC2TELE_AESYNC_START_ADDR+7));
...
static int zhuquec2tele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_zhuquec2tele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	default:
		break;
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, ZHUQUEC2TELE_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	u32 *feature_return_para_32 = (u32 *)para;
	*len = ZHUQUEC2TELE_UNIQUE_SENSOR_ID_LENGTH;
	memcpy(feature_return_para_32, zhuquec2tele_unique_id,
		ZHUQUEC2TELE_UNIQUE_SENSOR_ID_LENGTH);
...
static void read_unique_sensorid(struct subdrv_ctx *ctx)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		mdelay(1);
		if (adaptor_i2c_rd_p8(ctx->i2c_client, ctx->i2c_write_id >> 1, ZHUQUEC2TELE_UNIQUE_SENSOR_ID_ADDR,
			&(zhuquec2tele_unique_id[0]), ZHUQUEC2TELE_UNIQUE_SENSOR_ID_LENGTH) < 0) {
...
static int open(struct subdrv_ctx *ctx)
{
	...
	if ((module_flag & 0xFF00) == 0x0300){
		...
		subdrv_i2c_wr_regs_u16(ctx, zhuquec2tele_sensor_init_pre_setting1, ARRAY_SIZE(zhuquec2tele_sensor_init_pre_setting1));
		mdelay(5);
		subdrv_i2c_wr_regs_u16(ctx, zhuquec2tele_sensor_init_pre_setting2, ARRAY_SIZE(zhuquec2tele_sensor_init_pre_setting2));
		mdelay(5);
		zhuquec2tele_i2c_burst_wr_regs_u16(ctx, zhuquec2tele_sensor_simple_init_setting, ARRAY_SIZE(zhuquec2tele_sensor_simple_init_setting));
		...
		subdrv_i2c_wr_regs_u16(ctx, zhuquec2tele_sensor_init_pre_setting2, ARRAY_SIZE(zhuquec2tele_sensor_init_pre_setting2));
		mdelay(5);
		subdrv_i2c_wr_regs_u16(ctx, zhuquec2tele_sensor_init_setting, ARRAY_SIZE(zhuquec2tele_sensor_init_setting));
		DRV_LOGE(ctx, "setting end\n");
	};
...
//	ctx->extend_frame_length_en = FALSE;
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			zhuquec2tele_set_multi_shutter_frame_length(ctx, (u64*)&ae_ctrl->exposure, exp_cnt, 0);
			zhuquec2tele_set_multi_gain(ctx, (u32*)&ae_ctrl->gain, exp_cnt);
			break;
		case HDR_RAW_LBMF:
			zhuquec2tele_set_multi_shutter_frame_length_in_lut_convert(ctx,
				(u64 *)&ae_ctrl->exposure, exp_cnt, 0, frame_length_in_lut);
			set_multi_gain_in_lut(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);//same with konka
			break;
		default:
			zhuquec2tele_set_shutter_convert(ctx, ae_ctrl->exposure.le_exposure);
			zhuquec2tele_set_gain_convert(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
...
void zhuquec2tele_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
static void zhuquec2tele_set_hdr_tri_gain(struct subdrv_ctx *ctx, u64 *gains, u16 exp_cnt)
{
	...
	zhuquec2tele_set_multi_gain(ctx,	values, exp_cnt);
}

static int zhuquec2tele_set_hdr_tri_gain2(struct subdrv_ctx *ctx, u8 *para, u32 *len) {

	u64 *feature_data = (u64 *) para;
	zhuquec2tele_set_hdr_tri_gain(ctx, feature_data, 2);
	return 0;
}

static int zhuquec2tele_set_hdr_tri_gain3(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	u64 *feature_data = (u64 *) para;
	zhuquec2tele_set_hdr_tri_gain(ctx, feature_data, 3);
	return 0;
}
...
//	/* - (3) readout time cannot be overlapped */
	...
	if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
		zhuquec2tele_write_frame_length(ctx, ctx->frame_length);
	/* write shutter */
	switch (exp_cnt) {
	case 1:
	...
	zhuquec2tele_lens_pos_writeback(ctx);
...
static int zhuquec2tele_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	zhuquec2tele_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u64) (*(feature_data + 1)), (u64) (*(feature_data + 2)));
	return 0;
}

static void zhuquec2tele_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u64 values[3] = {0};
	u32 frame_length_in_lut[IMGSENSOR_STAGGER_EXPOSURE_CNT] = {0};
	...
	zhuquec2tele_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int zhuquec2tele_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	zhuquec2tele_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int zhuquec2tele_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	zhuquec2tele_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}

bool zhuquec2tele_set_long_exposure(struct subdrv_ctx *ctx,  u64 shutter)
{
	u16 l_shift = 0;
	u8 exposure_margin = ctx->s_ctx.mode[ctx->current_scenario_id].exposure_margin ?
		ctx->s_ctx.mode[ctx->current_scenario_id].exposure_margin : ctx->s_ctx.exposure_margin;
...
static void zhuquec2tele_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	if(zhuquec2tele_set_long_exposure(ctx, shutter)) {
		goto exit;
	}
...
static int zhuquec2tele_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
 	zhuquec2tele_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
	return 0;
}

static void zhuquec2tele_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter)
{
    zhuquec2tele_set_shutter_frame_length_convert(ctx, shutter, 0);
}

static int zhuquec2tele_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	zhuquec2tele_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], 0);
	return 0;
}
...
static int zhuquec2tele_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	zhuquec2tele_set_gain_convert(ctx, gain);
	return 0;
}

void zhuquec2tele_set_dummy(struct subdrv_ctx *ctx)
{
//	bool gph = !ctx->is_seamless && (ctx->s_ctx.s_gph != NULL);
//
//	if (gph)
...
static int zhuquec2tele_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.mode[scenario_id].linelength == 0) {
 	...
 	if (ctx->s_ctx.mode[scenario_id].hdr_mode == HDR_RAW_LBMF) {
  		zhuquec2tele_set_max_framerate_in_lut_by_scenario(ctx, scenario_id, framerate);
  		return 0;
  	}
	...
	if (ctx->s_ctx.reg_addr_auto_extend ||
		...
		zhuquec2tele_set_dummy(ctx);
	}
	return 0;
}

void zhuquec2tele_set_max_framerate(struct subdrv_ctx *ctx, u16 framerate, bool min_framelength_en)
{
	u32 frame_length = 0;
...
void zhuquec2tele_extend_frame_length_convert(struct subdrv_ctx *ctx, u32 ns)
{
	return ;
//	int i;
//	u32 last_exp_cnt = 1;
...
static int zhuquec2tele_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	zhuquec2tele_extend_frame_length_convert(ctx, ns);
	return 0;
}

bool zhuquec2tele_set_auto_flicker(struct subdrv_ctx *ctx, bool min_framelength_en)
{
	u16 framerate = 0;
	...
	if (framerate > 592 && framerate <= 607)
		zhuquec2tele_set_max_framerate(ctx, 592, min_framelength_en);
	else if (framerate > 296 && framerate <= 305)
		zhuquec2tele_set_max_framerate(ctx, 296, min_framelength_en);
	else if (framerate > 246 && framerate <= 253)
		zhuquec2tele_set_max_framerate(ctx, 246, min_framelength_en);
	else if (framerate > 236 && framerate <= 243)
		zhuquec2tele_set_max_framerate(ctx, 236, min_framelength_en);
	else if (framerate > 146 && framerate <= 153)
		zhuquec2tele_set_max_framerate(ctx, 146, min_framelength_en);
	else
		return FALSE;
	return TRUE;
}
...
static int zhuquec2tele_set_video_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (!framerate)
		return 0;
	zhuquec2tele_set_max_framerate(ctx, framerate, 0);
	zhuquec2tele_set_auto_flicker(ctx, 1);
	zhuquec2tele_set_dummy(ctx);
...
#define ZHUQUEC2TELE_AF_READ_ID  (0x1F)  //DW9827C
#define ZHUQUEC2TELE_AF_POSITON_ADD  (0x00)
...
static bool read_af_pos(struct subdrv_ctx *ctx, u16 *positon)
{
	...
	buf[0] = ZHUQUEC2TELE_AF_POSITON_ADD;

	msg[0].addr = ZHUQUEC2TELE_AF_READ_ID >> 1;
	msg[0].flags = i2c_client->flags;
	msg[0].buf = buf;
	msg[0].len = 1;

	msg[1].addr  = ZHUQUEC2TELE_AF_READ_ID >> 1;
...
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id) {
	struct subdrv_entry *m_subdrv_entry = &zhuquec2tele_mipi_raw_entry;
...
static int zhuquec2tele_get_ois_compensation(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	...
	u8  *ois_compensation = (u8 *)(uintptr_t)(*(feature_data + 1));
	if (*sensor_mode < ARRAY_SIZE(mode_struct)) {
		*ois_compensation = mode_struct[*sensor_mode].ois_compensation;
	}
	return 0;
}

static u32 ois_otp_info[] = {
	0x124E, /* multi_golden_addr_offset */
	10,     /* multi_golden_size */
	0x124E, /* multi_golden_valid_offset */
	0x1253, /* gradient_uint8_offset */
	0x1254, /* gradient_float_offset */
	0x1960, /* ois_calibration_addr_offset */
	2340,   /* ois_calibration_size */
	0x2284, /* ois_calibration_valid_offset */
	9,      /* ois_calibration_index_num */
	130,    /* ois_calibration_table_num */
	1,      /* is_ois_otp_data_exist */
};

static int zhuquec2tele_get_ois_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	u64 *feature_data = (u64 *)para;

	u32 *p_ois_otp_info = (u32 *)(uintptr_t)(*feature_data);
	memcpy(p_ois_otp_info, ois_otp_info, sizeof(ois_otp_info));
    pr_info("%s ois_otp_info size(%zu)",
			__func__, sizeof(ois_otp_info)/sizeof(u32));
	return 0;
}
static void zhuquec2tele_write_frame_length_in_lut(struct subdrv_ctx *ctx, u32 fll, u32 *fll_in_lut)
{
	...
	u64 *shutters, u16 exp_cnt, u32 frame_length, u32 *frame_length_in_lut)
{
	...
	zhuquec2tele_write_frame_length_in_lut(ctx, ctx->frame_length, ctx->frame_length_in_lut);
