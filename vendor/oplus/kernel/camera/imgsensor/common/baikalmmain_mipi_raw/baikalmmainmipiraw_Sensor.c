 ...
 *============================================================================
 ****************************************************************************/
#include "baikalmmainmipiraw_Sensor.h"

#define BAIKALMMAIN_EEPROM_READ_ID	0xA1
#define BAIKALMMAIN_EEPROM_WRITE_ID	0xA0
#define BAIKALMMAIN_MAX_OFFSET		0x8000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "baikalmmain_camera_sensor"
...
#define baikalmmain_burst_write_cmos_sensor(...) subdrv_i2c_wr_p8(__VA_ARGS__)
...
#define BAIKALMMAIN_UNIQUE_SENSOR_ID_ADDR    (0x7000)
#define BAIKALMMAIN_UNIQUE_SENSOR_ID_LENGTH  (16)
static BYTE baikalmmain_unique_id[BAIKALMMAIN_UNIQUE_SENSOR_ID_LENGTH] = { 0 };
...
static int baikalmmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int baikalmmain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void get_sensor_cali(void* arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static void baikalmmain_write_shutter(struct subdrv_ctx *ctx);
static int baikalmmain_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void baikalmmain_write_frame_length(struct subdrv_ctx *ctx, u32 fll);
static int baikalmmain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_get_cloud_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmain_set_cali_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
/* STRUCT */

static BYTE baikalmmain_common_data[OPLUS_CAMERA_COMMON_DATA_LENGTH] = { 0 };
...
static struct comp_mode_tran_time_params baikalmmain_comp_params[SENSOR_SCENARIO_ID_MAX] = {
	{ .enable = 0, }, /*pre*/
...
static struct eeprom_map_info baikalmmain_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, baikalmmain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, baikalmmain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, baikalmmain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_DATA, baikalmmain_get_sensor_sn},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, baikalmmain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, baikalmmain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, baikalmmain_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, baikalmmain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, baikalmmain_set_awb_gain},
	{SENSOR_FEATURE_SET_GAIN, baikalmmain_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, baikalmmain_set_shutter},
	{SENSOR_FEATURE_SET_FRAMELENGTH, baikalmmain_set_frame_length},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, baikalmmain_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, baikalmmain_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, baikalmmain_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, baikalmmain_streaming_resume},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, baikalmmain_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_GET_UNIQUE_SENSORID, baikalmmain_get_unique_sensorid},
	{SENSOR_FEATURE_GET_CLOUD_OTP_INFO, baikalmmain_get_cloud_otp_info},
	{SENSOR_FEATURE_SET_CALI_DATA, baikalmmain_set_cali_data},
};
...
static struct subdrv_mode_struct mode_struct[] = {
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = baikalmmain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_preview_setting),
	...
	{/*Reg_A_QBIN(VBIN)_4096x3072_30FPS with PDAF VB_max*/
		...
		.mode_setting_table = baikalmmain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = baikalmmain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_60FPS**/
		...
		.mode_setting_table = baikalmmain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = baikalmmain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = baikalmmain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_A-1_QBIN(VBIN)_4096x3072_24FPS with PDAF VB_max**/
		...
		.mode_setting_table = baikalmmain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = baikalmmain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = baikalmmain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmain_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = BAIKALMMAIN_SENSOR_ID,
	...
	.ana_gain_type = 1,
	.ana_gain_step = 1,
	.ana_gain_table = baikalmmain_ana_gain_table,
	.ana_gain_table_size = sizeof(baikalmmain_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = baikalmmain_init_setting,
	.init_setting_len = ARRAY_SIZE(baikalmmain_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry baikalmmain_mipi_raw_entry = {
	.name = "baikalmmain_mipi_raw",
	.id = BAIKALMMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_baikalmmain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != baikalmmain_eeprom_info[meta_id].meta)
		return -1;

	if (size != baikalmmain_eeprom_info[meta_id].size)
		return -1;

	addr = baikalmmain_eeprom_info[meta_id].start;
	readsize = baikalmmain_eeprom_info[meta_id].size;
...
static void read_unique_sensorid(struct subdrv_ctx *ctx)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		msleep(5);
		if (adaptor_i2c_rd_p8(ctx->i2c_client, ctx->i2c_write_id >> 1, BAIKALMMAIN_UNIQUE_SENSOR_ID_ADDR,
			&(baikalmmain_unique_id[0]), BAIKALMMAIN_UNIQUE_SENSOR_ID_LENGTH) < 0) {
...
static int baikalmmain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	*len = BAIKALMMAIN_UNIQUE_SENSOR_ID_LENGTH;
	memcpy(feature_return_para_32, baikalmmain_unique_id,
		BAIKALMMAIN_UNIQUE_SENSOR_ID_LENGTH);
...
static int baikalmmain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(feature_return_para_32, baikalmmain_common_data,
...
/* static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, BAIKALMMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
} */
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, BAIKALMMAIN_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, BAIKALMMAIN_EEPROM_READ_ID >> 1, reg, (BAIKALMMAIN_EEPROM_WRITE_ID & 0xFE) | 0x01);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, BAIKALMMAIN_EEPROM_READ_ID >> 1, reg, BAIKALMMAIN_EEPROM_WRITE_ID & 0xFE);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == BAIKALMMAIN_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
            /*&& (data_base == BAIKALMMAIN_STEREO_START_ADDR )*/) {
            ...
            */
        } else if ((pStereodata->uSensorId == BAIKALMMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            /*&& (data_base == BAIKALMMAIN_AESYNC_START_ADDR)*/) {
            ...
            /* LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, BAIKALMMAIN_AESYNC_START_ADDR+7));
        ...
        LOG_INF("baikalmmain write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int baikalmmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
    ...
    read_baikalmmain_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
            (BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, BAIKALMMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "baikalmmain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "baikalmmain read_otp_info end\n");
}
...
static void baikalmmain_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	baikalmmain_write_shutter(ctx);
}
...
static int baikalmmain_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			baikalmmain_set_shutter_convert(ctx, (u32 *)para);
...
static u16 baikalmmain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	if (ctx->current_scenario_id == SENSOR_SCENARIO_ID_CUSTOM3 || // RMSC
		...
		baikalmmain_feedback_awbgain[5] = r_gain_int;
		baikalmmain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
		baikalmmain_feedback_awbgain[9] = b_gain_int;
		baikalmmain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
		subdrv_i2c_wr_regs_u8(ctx, baikalmmain_feedback_awbgain,
			ARRAY_SIZE(baikalmmain_feedback_awbgain));
	}
}
...
static void baikalmmain_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
		u64 *shutter, u16 exp_cnt,	u16 frame_length)
{
	...
	baikalmmain_write_frame_length(ctx, ctx->frame_length);
	/* write shutter */
	switch (exp_cnt) {
	case 1:
...
static int baikalmmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmmain_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static void baikalmmain_set_dummy(struct subdrv_ctx *ctx)
{
	bool gph = !ctx->is_seamless && (ctx->s_ctx.s_gph != NULL);
...
static void baikalmmain_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
...
static void baikalmmain_write_shutter(struct subdrv_ctx *ctx)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps > 297 && realtime_fps <= 305) {
			baikalmmain_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps > 147 && realtime_fps <= 150) {
			baikalmmain_set_max_framerate(ctx, 146, 0);
		}
	}
	if (gph)
		ctx->s_ctx.s_gph((void *)ctx, 1);
...
static void baikalmmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
...
static int baikalmmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmmain_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}

static void baikalmmain_write_pdc_data(struct subdrv_ctx *ctx)
{
	DRV_LOG(ctx, "E!\n");

	if (ctx->s_ctx.eeprom_info->preload_pdc_table) {
		baikalmmain_burst_write_cmos_sensor(ctx, SENSOR_PDC_ADDR, &ctx->s_ctx.eeprom_info->preload_pdc_table[8], OTP_PDC_SIZE - 8);
		DRV_LOG(ctx, "Cloud burning Data Writing!\n");
	}
	else {
		baikalmmain_burst_write_cmos_sensor(ctx, SENSOR_PDC_ADDR, &otp_pdc_data[8], OTP_PDC_SIZE - 8);
...
static int open(struct subdrv_ctx *ctx)
{
	...
	sensor_init(ctx);
	/* PDC setting */
	baikalmmain_write_pdc_data(ctx);
	/*QSC&SPC setting*/
...
void baikalmmain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int baikalmmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	baikalmmain_get_min_shutter_by_scenario(ctx,
...
static void comp_mode_tran_time_cal1(struct subdrv_ctx *ctx, u32 scenario_id, u32* prsh) {
	...
	*prsh = 0U;
	if (baikalmmain_comp_params[ctx->current_scenario_id].clock_vtpxck == 0) {
	...
	system_delay = SYSTEM_DELAY1 * 1000 * 1000 * 10 /
		baikalmmain_comp_params[ctx->current_scenario_id].clock_vtpxck;
