 ...
 *============================================================================
 ****************************************************************************/
#include "baikalmfront2mipiraw_Sensor.h"

#define BAIKALMFRONT2_EEPROM_READ_ID	0xA9
#define BAIKALMFRONT2_EEPROM_WRITE_ID   0xA8
#define BAIKALMFRONT2_MAX_OFFSET		0x4000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "baikalmfront2_camera_sensor"
...
static int baikalmfront2_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_get_eeprom_4cell_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int close(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void set_sensor_cali(void *arg);
static int baikalmfront2_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront2_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int baikalmfront2_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static unsigned int read_baikalmfront2_eeprom_4cell_info(struct subdrv_ctx *ctx,
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, baikalmfront2_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, baikalmfront2_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, baikalmfront2_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, baikalmfront2_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, baikalmfront2_streaming_resume},
	{SENSOR_FEATURE_SET_ESHUTTER, baikalmfront2_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, baikalmfront2_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_GAIN, baikalmfront2_set_gain},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, baikalmfront2_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, baikalmfront2_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, baikalmfront2_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, baikalmfront2_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, gc32e2_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_GET_4CELL_DATA, baikalmfront2_get_eeprom_4cell_info},
};

static struct eeprom_info_struct eeprom_info[] = {
	{
		.header_id = 0x0401007F,
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = baikalmfront2_preview_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_preview_setting),
	...
	},
	{
		...
		.mode_setting_table = baikalmfront2_capture_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront2_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront2_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront2_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront2_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront2_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront2_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront2_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = BAIKALMFRONT2_SENSOR_ID,
	...
	.ana_gain_step = 1,
	.ana_gain_table = baikalmfront2_ana_gain_table,
	.ana_gain_table_size = sizeof(baikalmfront2_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = baikalmfront2_sensor_init_setting,
	.init_setting_len =  ARRAY_SIZE(baikalmfront2_sensor_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry baikalmfront2_mipi_raw_entry = {
	.name = "baikalmfront2_mipi_raw",
	.id = BAIKALMFRONT2_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void baikalmfront2_set_dummy(struct subdrv_ctx *ctx)
{
...
static void baikalmfront2_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
...
static void baikalmfront2_write_shutter(struct subdrv_ctx *ctx)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps > 592 && realtime_fps <= 607) {
			baikalmfront2_set_max_framerate(ctx, 592, 0);
		} else if (realtime_fps > 296 && realtime_fps <= 305) {
			baikalmfront2_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps > 246 && realtime_fps <= 253) {
			baikalmfront2_set_max_framerate(ctx, 246, 0);
		} else if (realtime_fps > 236 && realtime_fps <= 243) {
			baikalmfront2_set_max_framerate(ctx, 236, 0);
		} else if (realtime_fps > 146 && realtime_fps <= 153) {
			baikalmfront2_set_max_framerate(ctx, 146, 0);
		} else {
			// Extend frame length
			subdrv_i2c_wr_u16(ctx, 0x0340, ctx->frame_length);
		}
...
static void baikalmfront2_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	baikalmfront2_write_shutter(ctx);
}

static int baikalmfront2_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	baikalmfront2_set_shutter_frame_length(ctx, para, len);
	return 0;
}
...
static int baikalmfront2_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			baikalmfront2_set_shutter_convert(ctx, (u32 *)para);
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, BAIKALMFRONT2_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
			...
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			if (*sensor_id == 0x32E2) {
				*sensor_id = ctx->s_ctx.sensor_id;
				if (first_read) {
					read_baikalmfront2_eeprom_4cell_info(ctx, &oplus_eeprom_4cell_info, oplus_eeprom_4cell_addr_table);
...
static int open(struct subdrv_ctx *ctx)
{
	...
	if (get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
		return ERROR_SENSOR_CONNECT_FAIL;
	i2c_table_write(ctx, baikalmfront2_sensor_preinit_setting, ARRAY_SIZE(baikalmfront2_sensor_preinit_setting));
	mdelay(5);
	sensor_init(ctx);
...
static void baikalmfront2_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
static int baikalmfront2_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmfront2_set_multi_shutter_frame_length(ctx, (u32 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static void baikalmfront2_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u32 values[3] = {0};
	...
	baikalmfront2_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int baikalmfront2_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmfront2_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int baikalmfront2_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmfront2_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}
