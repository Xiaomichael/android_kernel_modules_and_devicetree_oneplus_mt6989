 ...
 *============================================================================
 ****************************************************************************/
#include "casiopmainmipiraw_Sensor.h"

#define CASIOPMAIN_EEPROM_READ_ID	0xA1
#define CASIOPMAIN_EEPROM_WRITE_ID	0xA0
#define CASIOPMAIN_MAX_OFFSET		0x8000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "casiopmain_camera_sensor"
...
static int casiopmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int casiopmain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void get_sensor_cali(void* arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static void casiopmain_write_shutter(struct subdrv_ctx *ctx);
static int casiopmain_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casiopmain_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void casiopmain_write_frame_length(struct subdrv_ctx *ctx, u32 fll);
/* STRUCT */

static BYTE casiopmain_common_data[OPLUS_CAMERA_COMMON_DATA_LENGTH] = { 0 };
...
static struct comp_mode_tran_time_params casiopmain_comp_params[SENSOR_SCENARIO_ID_MAX] = {
	{ .enable = 0, }, /*pre*/
...
static struct eeprom_map_info casiopmain_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, casiopmain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, casiopmain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, casiopmain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_DATA, casiopmain_get_sensor_sn},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, casiopmain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, casiopmain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, casiopmain_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, casiopmain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, casiopmain_set_awb_gain},
	{SENSOR_FEATURE_SET_GAIN, casiopmain_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, casiopmain_set_shutter},
	{SENSOR_FEATURE_SET_FRAMELENGTH, casiopmain_set_frame_length},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, casiopmain_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, casiopmain_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, casiopmain_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, casiopmain_streaming_resume},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, casiopmain_set_max_framerate_by_scenario},
};
...
static struct subdrv_mode_struct mode_struct[] = {
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casiopmain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_preview_setting),
	...
	{/*Reg_A_QBIN(VBIN)_4096x3072_30FPS with PDAF VB_max*/
		...
		.mode_setting_table = casiopmain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casiopmain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_60FPS**/
		...
		.mode_setting_table = casiopmain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casiopmain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casiopmain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_A-1_QBIN(VBIN)_4096x3072_24FPS with PDAF VB_max**/
		...
		.mode_setting_table = casiopmain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casiopmain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casiopmain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(casiopmain_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CASIOPMAIN_SENSOR_ID,
	...
	.ana_gain_type = 1,
	.ana_gain_step = 1,
	.ana_gain_table = casiopmain_ana_gain_table,
	.ana_gain_table_size = sizeof(casiopmain_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = casiopmain_init_setting,
	.init_setting_len = ARRAY_SIZE(casiopmain_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry casiopmain_mipi_raw_entry = {
	.name = "casiopmain_mipi_raw",
	.id = CASIOPMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_casiopmain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != casiopmain_eeprom_info[meta_id].meta)
		return -1;

	if (size != casiopmain_eeprom_info[meta_id].size)
		return -1;

	addr = casiopmain_eeprom_info[meta_id].start;
	readsize = casiopmain_eeprom_info[meta_id].size;
...
static int casiopmain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(feature_return_para_32, casiopmain_common_data,
...
/* static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CASIOPMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
} */
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CASIOPMAIN_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CASIOPMAIN_EEPROM_READ_ID >> 1, reg, (CASIOPMAIN_EEPROM_WRITE_ID & 0xFE) | 0x01);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CASIOPMAIN_EEPROM_READ_ID >> 1, reg, CASIOPMAIN_EEPROM_WRITE_ID & 0xFE);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == CASIOPMAIN_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
            /*&& (data_base == CASIOPMAIN_STEREO_START_ADDR )*/) {
            ...
            */
        } else if ((pStereodata->uSensorId == CASIOPMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            /*&& (data_base == CASIOPMAIN_AESYNC_START_ADDR)*/) {
            ...
            /* LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, CASIOPMAIN_AESYNC_START_ADDR+7));
        ...
        LOG_INF("casiopmain write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int casiopmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
    ...
    read_casiopmain_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
            (BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CASIOPMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "casiopmain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "casiopmain read_otp_info end\n");
}
...
static void casiopmain_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	casiopmain_write_shutter(ctx);
}
...
static int casiopmain_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			casiopmain_set_shutter_convert(ctx, (u32 *)para);
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			printk("cdb_test i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",
			...
			printk("cdb_test Read sensor id fail. i2c_write_id: 0x%x\n", ctx->i2c_write_id);
			printk("cdb_test sensor_id = 0x%x, ctx->s_ctx.sensor_id = 0x%x\n",
				*sensor_id, ctx->s_ctx.sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static u16 casiopmain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	if (ctx->current_scenario_id == SENSOR_SCENARIO_ID_CUSTOM3 || // RMSC
		...
		casiopmain_feedback_awbgain[5] = r_gain_int;
		casiopmain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
		casiopmain_feedback_awbgain[9] = b_gain_int;
		casiopmain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
		subdrv_i2c_wr_regs_u8(ctx, casiopmain_feedback_awbgain,
			ARRAY_SIZE(casiopmain_feedback_awbgain));
	}
}
...
static void casiopmain_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
		u64 *shutter, u16 exp_cnt,	u16 frame_length)
{
	...
	casiopmain_write_frame_length(ctx, ctx->frame_length);
	/* write shutter */
	switch (exp_cnt) {
	case 1:
...
static int casiopmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	casiopmain_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static void casiopmain_set_dummy(struct subdrv_ctx *ctx)
{
	bool gph = !ctx->is_seamless && (ctx->s_ctx.s_gph != NULL);
...
static void casiopmain_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
...
static void casiopmain_write_shutter(struct subdrv_ctx *ctx)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps > 297 && realtime_fps <= 305) {
			casiopmain_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps > 147 && realtime_fps <= 150) {
			casiopmain_set_max_framerate(ctx, 146, 0);
		}
	}
	if (gph)
		ctx->s_ctx.s_gph((void *)ctx, 1);
...
static void casiopmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
   ...
   printk( "cdb_test shutter =%x:%x:%x\n", (ctx->exposure[0] >> 16) & 0xFF, (ctx->exposure[0] >>  8) & 0xFF, (ctx->exposure[0])  & 0xFF);
   printk( "cdb_test read shutter  %x:%x:%x \n ", subdrv_i2c_rd_u8(ctx, 0x3500), subdrv_i2c_rd_u8(ctx, 0x3501), subdrv_i2c_rd_u8(ctx, 0x3502));
...
static int casiopmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	casiopmain_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	if (get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
		return ERROR_SENSOR_CONNECT_FAIL;
	printk( "cdb_test addr:0x0301 value:0x%x   line:%d", subdrv_i2c_rd_u8(ctx, 0x0301), __LINE__);
	subdrv_i2c_wr_u8(ctx, 0x0301, 0x01);
	printk("yuan addr:0x0301 value:0x%x   line:%d", subdrv_i2c_rd_u8(ctx, 0x0301), __LINE__);
	/* initail setting */
	printk( "cdb_test open addr:0x0103 value:0x%x  line:%d", subdrv_i2c_rd_u8(ctx, 0x0103), __LINE__);
...
void casiopmain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int casiopmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	casiopmain_get_min_shutter_by_scenario(ctx,
...
static void comp_mode_tran_time_cal1(struct subdrv_ctx *ctx, u32 scenario_id, u32* prsh) {
	...
	*prsh = 0U;
	if (casiopmain_comp_params[ctx->current_scenario_id].clock_vtpxck == 0) {
	...
	system_delay = SYSTEM_DELAY1 * 1000 * 1000 * 10 /
		casiopmain_comp_params[ctx->current_scenario_id].clock_vtpxck;
