 ...
 *============================================================================
 ****************************************************************************/
#include "caymancmainmipiraw_Sensor.h"

#define CAYMANCMAIN_EEPROM_READ_ID	0xA1
#define CAYMANCMAIN_EEPROM_WRITE_ID	0xA0
#define CAYMANCMAIN_MAX_OFFSET		0x8000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "caymancmain_camera_sensor"
...
//#define CAYMANCMAIN_UNIQUE_SENSOR_ID 0x0A1F
//#define CAYMANCMAIN_UNIQUE_SENSOR_ID_LENGHT 11
...
static int caymancmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
//static void caymancmain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int caymancmain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void get_sensor_cali(void* arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int caymancmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void caymancmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
/* STRUCT */

static BYTE caymancmain_common_data[OPLUS_CAMERA_COMMON_DATA_LENGTH] = { 0 };
//static BYTE caymancmain_unique_id[CAYMANCMAIN_UNIQUE_SENSOR_ID_LENGHT] = { 0 };
...
static struct comp_mode_tran_time_params caymancmain_comp_params[SENSOR_SCENARIO_ID_MAX] = {
	{ .enable = 0, }, /*pre*/
...
static struct eeprom_map_info caymancmain_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, caymancmain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, caymancmain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, caymancmain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_DATA, caymancmain_get_sensor_sn},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, caymancmain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, caymancmain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, caymancmain_get_otp_checksum_data},
	//{SENSOR_FEATURE_GET_UNIQUE_SENSORID, caymancmain_get_unique_sensorid},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, caymancmain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, caymancmain_set_awb_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, caymancmain_set_shutter},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{/*B8-S1 4096x3072 @30FPS QBIN(VBIN) w/ All-PD VB Max.*/
		.frame_desc = frame_desc_prev,
		.num_entries = ARRAY_SIZE(frame_desc_prev),
		.mode_setting_table = caymancmain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymancmain_seamless_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymancmain_seamless_preview),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3900,
	...
	{/*B8-S1 4096x3072 @30FPS QBIN(VBIN) w/ All-PD VB Max.*/
		...
		.mode_setting_table = caymancmain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_capture_setting),
	...
	{/*B6 4096x2304 @30FPS QBIN w/ Partial-PD VB Max.**/
		.frame_desc = frame_desc_vid,
		.num_entries = ARRAY_SIZE(frame_desc_vid),
		.mode_setting_table = caymancmain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*B7 4096x2304 @60FPS QBIN w/ Partial-PD VB Max.**/
		.frame_desc = frame_desc_hs,
		.num_entries = ARRAY_SIZE(frame_desc_hs),
		.mode_setting_table = caymancmain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*V1 2048x1152 @120FPS QBIN(VBIN) V2H2 w/ All-PD VB Max.**/
		.frame_desc = frame_desc_slim,
		.num_entries = ARRAY_SIZE(frame_desc_slim),
		.mode_setting_table = caymancmain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*V2 2048x1152 @240FPS QBIN V2H2 w/o PD VB Max.**/
		.frame_desc = frame_desc_cus1,
		.num_entries = ARRAY_SIZE(frame_desc_cus1),
		.mode_setting_table = caymancmain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*B2-S2 4096x3072 @24FPS QBIN(VBIN) w/ All-PD VB Max. Seamless F4-RAW-S2**/
		.frame_desc = frame_desc_cus2,
		.num_entries = ARRAY_SIZE(frame_desc_cus2),
		.mode_setting_table = caymancmain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom2_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = caymancmain_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymancmain_seamless_custom2),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 4876,
    ...
    {/*F2 8192x6144 @15FPS Full RMSC w/ Alll-PD VB Max.**/
		.frame_desc = frame_desc_cus3,
		.num_entries = ARRAY_SIZE(frame_desc_cus3),
		.mode_setting_table = caymancmain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*F3-RAW-S1 4096x3072 @30FPS Full RAW Crop w/ Alll-PD VB Max. Seamless w/ "*-S1"**/
		.frame_desc = frame_desc_cus4,
		.num_entries = ARRAY_SIZE(frame_desc_cus4),
		.mode_setting_table = caymancmain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymancmain_seamless_custom4,
		.seamless_switch_mode_setting_len =  ARRAY_SIZE(caymancmain_seamless_custom4),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3252,
	...
	{/*F4-RAW-S2 4096x3072 @24FPS Full RAW Crop w/ Alll-PD VB Max. Seamless B2-S2**/
		.frame_desc = frame_desc_cus5,
		.num_entries = ARRAY_SIZE(frame_desc_cus5),
		.mode_setting_table = caymancmain_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom5_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*B3 3840x2160 @30FPS QBIN(VBIN) w/ All-PD VB Max.**/
		.frame_desc = frame_desc_cus6,
		.num_entries = ARRAY_SIZE(frame_desc_cus6),
		.mode_setting_table = caymancmain_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*B4 3840x2160 @60FPS QBIN w/ Partial-PD VB Max. 11**/
		.frame_desc = frame_desc_cus7,
		.num_entries = ARRAY_SIZE(frame_desc_cus7),
		.mode_setting_table = caymancmain_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = caymancmain_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*4096x3072 @60FPS QBIN w/ Partial-PD VB Max. DataRate Min.*/
		.frame_desc = frame_desc_cus9,
		.num_entries = ARRAY_SIZE(frame_desc_cus9),
		.mode_setting_table = caymancmain_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = caymancmain_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = caymancmain_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(caymancmain_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CAYMANCMAIN_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = caymancmain_ana_gain_table,
	.ana_gain_table_size = sizeof(caymancmain_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = caymancmain_init_setting,
	.init_setting_len = ARRAY_SIZE(caymancmain_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry caymancmain_mipi_raw_entry = {
	.name = "caymancmain_mipi_raw",
	.id = CAYMANCMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_caymancmain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != caymancmain_eeprom_info[meta_id].meta)
		return -1;

	if (size != caymancmain_eeprom_info[meta_id].size)
		return -1;

	addr = caymancmain_eeprom_info[meta_id].start;
	readsize = caymancmain_eeprom_info[meta_id].size;
...
static int caymancmain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(feature_return_para_32, caymancmain_common_data,
...
static void caymancmain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(feature_return_para_32, caymancmain_unique_id,
		CAYMANCMAIN_UNIQUE_SENSOR_ID_LENGHT);
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CAYMANCMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CAYMANCMAIN_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANCMAIN_EEPROM_READ_ID >> 1, reg, (CAYMANCMAIN_EEPROM_WRITE_ID & 0xFE) | 0x01);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANCMAIN_EEPROM_READ_ID >> 1, reg, CAYMANCMAIN_EEPROM_WRITE_ID & 0xFE);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == CAYMANCMAIN_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
            && (data_base == CAYMANCMAIN_STEREO_START_ADDR )) {
            ...
            */
        } else if ((pStereodata->uSensorId == CAYMANCMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == CAYMANCMAIN_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, CAYMANCMAIN_AESYNC_START_ADDR+7));
        ...
        LOG_INF("caymancmain write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int caymancmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
    ...
    read_caymancmain_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
            (BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CAYMANCMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "caymancmain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "caymancmain read_otp_info end\n");
}
...
static u16 caymancmain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	if (ctx->current_scenario_id == SENSOR_SCENARIO_ID_CUSTOM3 || // RMSC
		...
		caymancmain_feedback_awbgain[5] = r_gain_int;
		caymancmain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
		caymancmain_feedback_awbgain[9] = b_gain_int;
		caymancmain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
		subdrv_i2c_wr_regs_u8(ctx, caymancmain_feedback_awbgain,
			ARRAY_SIZE(caymancmain_feedback_awbgain));
	}
}
...
void caymancmain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int caymancmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	caymancmain_get_min_shutter_by_scenario(ctx,
...
static void comp_mode_tran_time_cal1(struct subdrv_ctx *ctx, u32 scenario_id, u32* prsh) {
	...
	*prsh = 0U;
	if (caymancmain_comp_params[ctx->current_scenario_id].clock_vtpxck == 0) {
	...
	system_delay = SYSTEM_DELAY1 * 1000 * 1000 * 10 /
		caymancmain_comp_params[ctx->current_scenario_id].clock_vtpxck;
...
static int caymancmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	caymancmain_set_shutter_frame_length(ctx, para, len);
	return 0;
}

static int caymancmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	caymancmain_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
	return 0;
}

bool caymancmain_set_auto_flicker(struct subdrv_ctx *ctx, bool min_framelength_en)
{
	u16 framerate = 0;
	bool ret = TRUE;
	...
	case 1:
		if (framerate > 592 && framerate <= 607)
			set_max_framerate(ctx, 600, min_framelength_en);
...
static void caymancmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	/* write framelength */
	if (caymancmain_set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
