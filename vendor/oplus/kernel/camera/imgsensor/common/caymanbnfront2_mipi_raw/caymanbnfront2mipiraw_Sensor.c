 ...
 *============================================================================
 ****************************************************************************/
#include "caymanbnfront2mipiraw_Sensor.h"


#define CAYMANBNFRONT2_EEPROM_READ_ID	0xA9
...
#define PFX "caymanbnfront2_camera_sensor"
...
static int caymanbnfront2_open(struct subdrv_ctx *ctx);
static int caymanbnfront2_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
...
#ifdef CAYMANBNFRONT2_AOV_MCLK_26M
static int set_pwr_seq_reset_view_to_sensing(void *arg);
#endif /* CAYMANBNFRONT2_AOV_MCLK_26M */

static int caymanbnfront2_streaming_control(void *arg, bool enable);
static int caymanbnfront2_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnfront2_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnfront2_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static int caymanbnfront2_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void set_sensor_cali(void *arg);
static void get_sensor_cali(void* arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int caymanbnfront2_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);

/* STRUCT */
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, caymanbnfront2_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, caymanbnfront2_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, caymanbnfront2_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, caymanbnfront2_get_otp_checksum_data},
	// {SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, caymanbnfront2_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, caymanbnfront2_set_awb_gain},
};
...
static struct eeprom_info_struct eeprom_info[] = {
	{
		...
		.qsc_support = TRUE,
		.qsc_size = 0x0618,
		.addr_qsc = 0x1000, /* QSC_EEPROM_ADDR 0x1000~0x1617*/
...
static struct subdrv_mode_struct mode_struct[] = {
	{  /* B9-S7 3264x2448,30fps,2X2Binning,w/o PD,For MIPI(Around 868.8Mpps) */
		...
		.mode_setting_table = caymanbnfront2_preview_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnfront2_preview_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnfront2_preview_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 864000000,
	...
	{	/* B9-S7 3264x2448,30fps,2X2Binning,w/o PD,For MIPI(Around 868.8Mpps) */
		...
		.mode_setting_table = caymanbnfront2_capture_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_capture_setting),
	...
	{/*3264x1836 @30FPS BIN w/o PD DataRate 780Mbps/lane VB Max.*/
		...
		.mode_setting_table = caymanbnfront2_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg B7-S8, 3264x1856,60fps,2X2Binning,w/o PD,For seamless2 group*/
		...
		.mode_setting_table = caymanbnfront2_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg B7-S8, 3264x1856,60fps,2X2Binning,w/o PD,For seamless2 group*/
		...
		.mode_setting_table = caymanbnfront2_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg V2-4 1280x960,10fps,4x4Binning_Crop,w/o PD,For MIPI<1.5G(Skew off)&Hblank=2.4us*/
		...
		.mode_setting_table = caymanbnfront2_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg F1-S7, 6560x4928,15fps,Full,w/o PD,For seamless1 group*/
		...
		.mode_setting_table = caymanbnfront2_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_custom2_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnfront2_custom2_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnfront2_custom2_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 864000000,
	...
	{/*Reg_B8-3 3264x1856,15fps,2X2Binning,w/o PD,For MaxVB*/
		...
		.mode_setting_table = caymanbnfront2_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg V3 1640x1232,30fps,4x4Binning,w/o PD,For MaxVB&Binning-Ave*/
		...
		.mode_setting_table = caymanbnfront2_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnfront2_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CAYMANBNFRONT2_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = caymanbnfront2_ana_gain_table,
	.ana_gain_table_size = sizeof(caymanbnfront2_ana_gain_table),
	.exposure_def = 0x3D0,
	.exposure_min = 16,
...
#endif
	.s_streaming_control = caymanbnfront2_streaming_control,
	...
	.init_setting_table = caymanbnfront2_init_setting,
	.init_setting_len = ARRAY_SIZE(caymanbnfront2_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	.init_ctx = init_ctx,
	.open = caymanbnfront2_open,
	.get_id = caymanbnfront2_get_imgsensor_id,
...
const struct subdrv_entry caymanbnfront2_mipi_raw_entry = {
	.name = "caymanbnfront2_mipi_raw",
	.id = CAYMANBNFRONT2_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int caymanbnfront2_open(struct subdrv_ctx *ctx)
{
	...
	DRV_LOG_MUST(ctx, "for caymanbnfront2 start\n");
	...
	/* get sensor id */
	if (caymanbnfront2_get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
...
static int caymanbnfront2_streaming_control(void *arg, bool enable) {
	...
	DRV_LOG_MUST(ctx,
		"caymanbnfront2 streaming_enable(0=Sw Standby,1=streaming):(%d)\n", enable);
	...
	if (enable) { // stream on
		...
		if (stream_refcnt_for_aov) {
			...
			if (ret < 0) {
				DRV_LOG_MUST(ctx,
					"caymanbnfront2 select(%s)(fail),ret(%d)\n",
					state_names[STATE_SCL_AP], ret);
				return ret;
			}
			DRV_LOG(ctx, "caymanbnfront2 select(%s)(correct)\n", state_names[STATE_SCL_AP]);
...
{
	struct subdrv_ctx *ctx = (struct subdrv_ctx *)arg;
	DRV_LOGE(ctx, "caymanbnfront2 set_pwr_seq_reset_view_to_sensing");
...
static int caymanbnfront2_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return caymanbnfront2_get_imgsensor_id(ctx, (u32 *)para);
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CAYMANBNFRONT2_EEPROM_READ_ID >> 1,
		addr, data, size) < 0) {
		DRV_LOGE(ctx, "caymanbnfront2 read_cmos_eeprom_p8 failed\n");
		return false;
	}
	DRV_LOGE(ctx, "caymanbnfront2 read_cmos_eeprom_p8 success read size = %d\n", size);
	return true;
}

static void read_otp_info(struct subdrv_ctx *ctx)
{
	DRV_LOGE(ctx, "caymanbnfront2 read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "caymanbnfront2 read_otp_info end\n");
}
