 ...
 *============================================================================
 ****************************************************************************/
#include "caymanbnmainmipiraw_Sensor.h"

#define CAYMANBNMAIN_EEPROM_READ_ID	0xA0
#define CAYMANBNMAIN_EEPROM_WRITE_ID	0xA1
#define CAYMANBNMAIN_AF_WRITE_ID	0x18
#define CAYMANBNMAIN_MAX_OFFSET		0x8000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "caymanbnmain_camera_sensor"
...
static int caymanbnmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int caymanbnmain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int caymanbnmain_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbnmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void caymanbnmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static int caymanbnmain_get_otp_qcom_pdaf_offset_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void calculate_prsh_length_lines(struct subdrv_ctx *ctx,
	struct mtk_hdr_ae *ae_ctrl,
	enum SENSOR_SCENARIO_ID_ENUM scenario_id);
/* STRUCT */

static struct eeprom_map_info caymanbnmain_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, caymanbnmain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, caymanbnmain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, caymanbnmain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, caymanbnmain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, caymanbnmain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, caymanbnmain_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, caymanbnmain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, caymanbnmain_set_awb_gain},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, caymanbnmain_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_OFFSET_DATA, caymanbnmain_get_otp_qcom_pdaf_offset_data},
	{SENSOR_FEATURE_SET_ESHUTTER, caymanbnmain_set_shutter},
};

static u32 caymanbnmain_dcg_ratio_table_ratio4[] = {4000};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4Crop = {
		...
		{0, 384}, {0, 0}, {2048, 1536}, {0, 0}, {0, 0},
		/* <cust21> <cust22> <cust23>*/
		{256, 912}, {0, 384}, {128, 456},
	},
...
static struct subdrv_mode_struct mode_struct[] = {
	{/* B1-S1 4096x3072 @30fps QBIN(VBIN) with PDAF VB_max seamless A-1*/
		...
		.mode_setting_table = caymanbnmain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_preview),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* B1-S1 4096x3072 @30fps QBIN(VBIN) with PDAF VB_max seamless A-1*/
		...
		.mode_setting_table = caymanbnmain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_capture_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_capture,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_capture),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/*B2-S2 4096x2304_30FPS**/
		...
		.mode_setting_table = caymanbnmain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_normal_video_setting),
	...
	{/* B3-1 4096x2304_60FPS */
		...
		.mode_setting_table = caymanbnmain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{   /* G2 4096x2304 @60FPS QBIN DCG-HDR RAW12 w/ Partial-PD VB_max*/
		...
		.mode_setting_table = caymanbnmain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		...
		.dcg_info = {
			...
			.dcg_gain_table = caymanbnmain_dcg_ratio_table_ratio4,
			.dcg_gain_table_size = sizeof(caymanbnmain_dcg_ratio_table_ratio4),
		},
		.dpc_enabled = true,
	...
	{/* V1  QBIN(VBIN)-V2H2 FHD 2048x1152_240FPS*/
		...
		.mode_setting_table = caymanbnmain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* B6-S3 QBIN(VBIN)_4096x3072 @24FPS */
		...
		.mode_setting_table = caymanbnmain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom2_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_custom2),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* F2-S1 8192x6144_30FPS remosaic*/
		...
		.mode_setting_table = caymanbnmain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom3_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_custom3,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_custom3),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/*L1-S1 QBIN(VBIN) 4096x3072_2-exp LBMF 30FPS with PDAF VB_max*/
		...
		.mode_setting_table = caymanbnmain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_custom4),
	...
	{/* F1-S1 QRMSC_4096x3072_30FPS for izoom--bayer */
		...
		.mode_setting_table = caymanbnmain_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom5_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_custom5,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_custom5),
		.hdr_mode = HDR_NONE,
		.pclk = 2196000000,
		.linelength = 11424,
		.framelength = 6400,
	...
	{/*V2 QBIN-V2H2 FHD 2048x1152_480FPS */
		...
		.mode_setting_table = caymanbnmain_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*L2-S1 4096x3072 @30FPS Izoom 2exp-LBMF Qbayer w/ PDAF  VB_max seamless reg_A-1*/
		...
		.mode_setting_table = caymanbnmain_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom7_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_custom7,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_custom7),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/*B4 4096x3072 @60FPS QBIN(VBIN) with PDAF VB_max*/
		...
		.mode_setting_table = caymanbnmain_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*G1-S2 4096x2304 30FPS DCG AP Merge*/
		...
		.mode_setting_table = caymanbnmain_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
	...
	{/* G3 4096x2304 60FPS DCG AP Merge */
		...
		.mode_setting_table = caymanbnmain_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_RAW,
	...
	{/* B8-S3 1856X1392,30.1fps,2X2Binning_Crop,w/All-PD(VBin) */
		...
		.mode_setting_table = caymanbnmain_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* F2-S1 8192x6144_30FPS Qbayer*/
		...
		.mode_setting_table = caymanbnmain_custom12_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom12_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_custom12,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_custom12),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/*not use B6-S3 QBIN(VBIN)_4096x3072 @24FPS */
		...
		.mode_setting_table = caymanbnmain_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom13_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* F1-S1 4096x3072 30FPS izoom - Qbayer*/
		...
		.mode_setting_table = caymanbnmain_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom14_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*not use V1 QBIN(VBIN)-V2H2 FHD 2048x1152_240FPS*/
		...
		.mode_setting_table = caymanbnmain_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom15_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* B5 4096x2304 @120FPS QBIN Partial-PD VB_max*/
		...
		.mode_setting_table = caymanbnmain_custom16_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom16_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* G4 4096x3072 30FPS DCG AP Merge */
		...
		.mode_setting_table = caymanbnmain_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom17_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_RAW,
	...
	{/*F4-S3 Izoom 2exp-LBMF 4096x3072 @24FPS w/ PDAF VB_max*/
		...
		.mode_setting_table = caymanbnmain_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom18_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_LBMF,
	...
	{/*L3-S3 4096X3072,24fps,2X2Binning_2exp_LB-MF,w/All-PD(VBin),For RST(9-16ms)*/
		...
		.mode_setting_table = caymanbnmain_custom19_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom19_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = caymanbnmain_seamless_custom19,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbnmain_seamless_custom19),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/* reg_B4 2560x1920 @24FPS Binning w/ All-PD VB Max. */
		.frame_desc = frame_desc_cus20,
		.num_entries = ARRAY_SIZE(frame_desc_cus20),
		.mode_setting_table = caymanbnmain_custom20_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom20_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*F2 7680x4320 @30FPS Full-Crop w/ All-PD VB Max.*/
		.frame_desc = frame_desc_cus21,
		.num_entries = ARRAY_SIZE(frame_desc_cus21),
		.mode_setting_table = caymanbnmain_custom21_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom21_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*reg_B7 4096x2304 @30FPS Binning w/ All-PD 14bit unpacked VB Max.*/
		.frame_desc = frame_desc_cus22,
		.num_entries = ARRAY_SIZE(frame_desc_cus22),
		.mode_setting_table = caymanbnmain_custom22_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom22_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*reg_B8 3840x2160 @120FPS Binning w/ Partial-PD VB Max.*/
		.frame_desc = frame_desc_cus23,
		.num_entries = ARRAY_SIZE(frame_desc_cus23),
		.mode_setting_table = caymanbnmain_custom23_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbnmain_custom23_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CAYMANBNMAIN_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = caymanbnmain_ana_gain_table,
	.ana_gain_table_size = sizeof(caymanbnmain_ana_gain_table),
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = caymanbnmain_init_setting,
	.init_setting_len = ARRAY_SIZE(caymanbnmain_init_setting),
	.mode = mode_struct,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_DVDD, 1080000, 4},
	{HW_ID_DOVDD, 1800000, 3},
	{HW_ID_MCLK_DRIVING_CURRENT, 4, 6},
	{HW_ID_RST, 1, 4}
};

const struct subdrv_entry caymanbnmain_mipi_raw_entry = {
	.name = "caymanbnmain_mipi_raw",
	.id = CAYMANBNMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_caymanbnmain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != caymanbnmain_eeprom_info[meta_id].meta)
		return -1;

	if (size != caymanbnmain_eeprom_info[meta_id].size)
		return -1;

	addr = caymanbnmain_eeprom_info[meta_id].start;
	readsize = caymanbnmain_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CAYMANBNMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, CAYMANBNMAIN_EEPROM_WRITE_ID >> 1,
			addr, para, len);
...
static kal_int32 write_1st_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	adaptor_i2c_wr_u8(ctx->i2c_client, (0xB0 | CAYMANBNMAIN_EEPROM_WRITE_ID) >> 1, reg, 0x0);

	reg = 0x06ca;
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, (0xB0 | CAYMANBNMAIN_EEPROM_WRITE_ID) >> 1, reg, 0x2);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, (0xB0 | CAYMANBNMAIN_EEPROM_WRITE_ID) >> 1, reg, 0x0);
	}

	adaptor_i2c_rd_u8(ctx->i2c_client, CAYMANBNMAIN_EEPROM_WRITE_ID >> 1, reg, &flag);
...
static kal_int32 write_2nd_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANBNMAIN_EEPROM_WRITE_ID >> 1, reg, 0x0E);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANBNMAIN_EEPROM_WRITE_ID >> 1, reg, 0x00);
	}
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	u8 flag = 0;
	int ret = 0;
	adaptor_i2c_rd_u8(ctx->i2c_client, CAYMANBNMAIN_EEPROM_READ_ID >> 1, 0x000D, &flag);
	if (flag == 0x01) {
	...
	ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if (((pStereodata->uSensorId == CAYMANBNMAIN_SENSOR_ID) && ((data_length - 2) == CALI_DATA_MASTER_LENGTH))
				&& (data_base == CAYMANBNMAIN_STEREO_START_ADDR || data_base == CAYMANBNMAIN_STEREO_MT_START_ADDR
				|| data_base == CAYMANBNMAIN_STEREO_MT_105CM_START_ADDR)) {
		...
		} else if ((pStereodata->uSensorId == CAYMANBNMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
				&& (data_base == CAYMANBNMAIN_AESYNC_START_ADDR)) {
			...
			LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR),
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR+1),
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR+2),
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR+3),
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR+4),
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR+5),
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR+6),
				read_cmos_eeprom_8(ctx, CAYMANBNMAIN_AESYNC_START_ADDR+7));
		...
		LOG_INF("caymanbnmain write_Module_data pStereodata is null\n");
		return -1;
	}
	return ret;
}
...
static int caymanbnmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_caymanbnmain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_caymanbnmain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MW_MAIN:
	default:
		read_caymanbnmain_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	}
	return 0;
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CAYMANBNMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "caymanbnmain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "caymanbnmain read_otp_info end\n");
}
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			printk("caymanbnmain_mipi_raw i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",
...
static u16 caymanbnmain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	caymanbnmain_feedback_awbgain[5] = r_gain_int;
	caymanbnmain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
	caymanbnmain_feedback_awbgain[9] = b_gain_int;
	caymanbnmain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
	subdrv_i2c_wr_regs_u8(ctx, caymanbnmain_feedback_awbgain,
		ARRAY_SIZE(caymanbnmain_feedback_awbgain));
}
...
static int caymanbn_adaptor_i2c_wr_u8_u8(struct i2c_client *i2c_client, u8 addr, u8 reg, u8 val)
{
	int ret;
	u8 buf[2];
	struct i2c_msg msg;
...
static int open(struct subdrv_ctx *ctx)
{
	...
	/* initail setting */
	sensor_init(ctx);
	ret = caymanbn_adaptor_i2c_wr_u8_u8(ctx->i2c_client, CAYMANBNMAIN_AF_WRITE_ID >> 1, 0x02, 0x0);
	mdelay(2);
	DRV_LOGE(ctx, "init caymanbnmain af before write qsc data, ret: %d\n", ret);
...
void caymanbnmain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
		enum SENSOR_SCENARIO_ID_ENUM scenario_id,
...
int caymanbnmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	caymanbnmain_get_min_shutter_by_scenario(ctx,
...
static int caymanbnmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	caymanbnmain_set_shutter_frame_length(ctx, para, len);
	return 0;
}
...
static int caymanbnmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	caymanbnmain_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
	return 0;
}

void caymanbnmain_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
static void caymanbnmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	if (ctx->current_scenario_id == SENSOR_SCENARIO_ID_CUSTOM23) {
		if (shutter < exposure_margin) {
			shutter = exposure_margin;
		}
	    shutter = max_t(u64, shutter - exposure_margin,
		         (u64)ctx->s_ctx.mode[ctx->current_scenario_id].multi_exposure_shutter_range[0].min);
	}
	...
	if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
		caymanbnmain_write_frame_length(ctx, ctx->frame_length);
