 ...
 *============================================================================
 ****************************************************************************/
#include "baikalmfront3mipiraw_Sensor.h"

#define PFX "baikalmfront3_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

#define BAIKALMFRONT3_EEPROM_READ_ID			0xA9
#define BAIKALMFRONT3_EEPROM_WRITE_ID			0xA8
#define BAIKALMFRONT3_MAX_OFFSET				0x4000
...
#define BAIKALMFRONT3_UNIQUE_SENSOR_ID_ADDR    (0x0A20)
#define BAIKALMFRONT3_UNIQUE_SENSOR_ID_LENGTH  (11)

static BYTE baikalmfront3_unique_id[BAIKALMFRONT3_UNIQUE_SENSOR_ID_LENGTH] = { 0 };
static kal_uint8 otp_data_checksum[OTP_SIZE] = {0};
static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int baikalmfront3_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void get_sensor_cali(void* arg);
static void set_sensor_cali(void *arg);
static int baikalmfront3_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int baikalmfront3_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_set_cali_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmfront3_get_cloud_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
/* STRUCT */

static struct eeprom_map_info baikalmfront3_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, baikalmfront3_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, baikalmfront3_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, baikalmfront3_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, baikalmfront3_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, baikalmfront3_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, baikalmfront3_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, baikalmfront3_streaming_resume},
	{SENSOR_FEATURE_SET_ESHUTTER, baikalmfront3_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, baikalmfront3_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_GAIN, baikalmfront3_set_gain},
	{SENSOR_FEATURE_GET_UNIQUE_SENSORID, baikalmfront3_get_unique_sensorid},
	{SENSOR_FEATURE_SET_CALI_DATA, baikalmfront3_set_cali_data},
	{SENSOR_FEATURE_GET_CLOUD_OTP_INFO, baikalmfront3_get_cloud_otp_info},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = baikalmfront3_preview_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_preview_setting),
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_capture_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmfront3_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmfront3_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = BAIKALMFRONT3_SENSOR_ID,
	...
	.ana_gain_step = 1,
	.ana_gain_table = baikalmfront3_ana_gain_table,
	.ana_gain_table_size = sizeof(baikalmfront3_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	.exposure_max = (0xffff -18) * 128,
	.exposure_step = 1,
	.exposure_margin = 18,
	...
	.init_setting_table = baikalmfront3_sensor_init_setting,
	.init_setting_len =  ARRAY_SIZE(baikalmfront3_sensor_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry baikalmfront3_mipi_raw_entry = {
	.name = "baikalmfront3_mipi_raw",
	.id = BAIKALMFRONT3_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void baikalmfront3_set_dummy(struct subdrv_ctx *ctx)
{
...
static void baikalmfront3_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
			kal_bool min_framelength_en)
{
	...
	baikalmfront3_set_dummy(ctx);
}

void baikalmfront3_set_long_exposure(struct subdrv_ctx *ctx)
{
	...
	if (shutter > (ctx->s_ctx.frame_length_max - ctx->s_ctx.exposure_margin)) {
		...
		subdrv_i2c_wr_u8(ctx, 0x0104, 0x01);
		subdrv_i2c_wr_u8(ctx, ctx->s_ctx.reg_addr_exposure_lshift, l_shift);
		subdrv_i2c_wr_u8(ctx, 0x0104, 0x00);
		ctx->l_shift = l_shift;
...
static void baikalmfront3_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305) {
			baikalmfront3_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps >= 147 && realtime_fps <= 150) {
			baikalmfront3_set_max_framerate(ctx, 146, 0);
		} else {
			// Extend frame length
			subdrv_i2c_wr_u8(ctx, 0x0104, 0x01);
			subdrv_i2c_wr_u8(ctx, 0x0340, ctx->frame_length >> 8);
...
static int baikalmfront3_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmfront3_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}

static void baikalmfront3_write_shutter(struct subdrv_ctx *ctx)
{
	kal_uint16 realtime_fps = 0;
	...
	/* Long Exposure */
	baikalmfront3_set_long_exposure(ctx);
...
static void baikalmfront3_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	baikalmfront3_write_shutter(ctx);
}

static int baikalmfront3_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *para);
	baikalmfront3_set_shutter_convert(ctx, (u32 *)para);
	return 0;
}
...
static unsigned int read_baikalmfront3_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != baikalmfront3_eeprom_info[meta_id].meta)
		return -1;

	if (size != baikalmfront3_eeprom_info[meta_id].size)
		return -1;

	addr = baikalmfront3_eeprom_info[meta_id].start;
	readsize = baikalmfront3_eeprom_info[meta_id].size;
...
static struct SENSOR_OTP_INFO_STRUCT cloud_otp_info[OPLUS_CAM_CAL_DATA_MAX] = {
	{
		.OtpInfoLen = 1,
		.OtpInfo = {{0x0000, 14}}, /*{addr_modinfo, addr_modinfolen}*/
	...
	{
		.OtpInfoLen = 5,
		.OtpInfo = {{0x0010, 16}, {0x0034, 16}, {0x0060, 4}, {0x006c, 4}, {0x0072, 6}},
	...
	{
		.OtpInfoLen = 8,
		.OtpInfo = {{0x0000, 17}, {0x0006, 2}, {0x0008, 2}, {0x000a, 2}, {0x0072, 7}, {0x0072, 2}, {0x0074, 2}, {0x00b0, 24}},
		.isAFCodeOffset = KAL_FALSE,
...
static void read_unique_sensorid(struct subdrv_ctx *ctx)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		if (adaptor_i2c_rd_p8(ctx->i2c_client, ctx->i2c_write_id >> 1, BAIKALMFRONT3_UNIQUE_SENSOR_ID_ADDR,
			&(baikalmfront3_unique_id[0]), BAIKALMFRONT3_UNIQUE_SENSOR_ID_LENGTH) < 0) {
...
static int baikalmfront3_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	*len = BAIKALMFRONT3_UNIQUE_SENSOR_ID_LENGTH;
	memcpy(feature_return_para_32, baikalmfront3_unique_id,
		BAIKALMFRONT3_UNIQUE_SENSOR_ID_LENGTH);
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
    ...
    adaptor_i2c_rd_u8(ctx->i2c_client, BAIKALMFRONT3_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
    return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, BAIKALMFRONT3_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, BAIKALMFRONT3_EEPROM_WRITE_ID >> 1, reg, 0xA1);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, BAIKALMFRONT3_EEPROM_WRITE_ID >> 1, reg, 0xA0);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == BAIKALMFRONT3_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
            && (data_base == BAIKALMFRONT3_STEREO_START_ADDR)) {
        ...
        } else if ((pStereodata->uSensorId == BAIKALMFRONT3_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == BAIKALMFRONT3_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, BAIKALMFRONT3_AESYNC_START_ADDR+7));
        ...
        LOG_INF("baikalmfront3 write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int baikalmfront3_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_baikalmfront3_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
			(BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, BAIKALMFRONT3_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "baikalmfront3 read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "baikalmfront3 read_otp_info end\n");
}
...
static int baikalmfront3_set_cali_data(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	...
	type = para[(*len) + 1];
	LOG_INF("set_cali_data flow enter");
	if (type == QSC_TYPE) {
		/* QSC data */
		support = info[idx].qsc_support;
	...
	LOG_INF("set_cali_data flow exit");
...
int baikalmfront3_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64* feature_data = (u64*)para;
