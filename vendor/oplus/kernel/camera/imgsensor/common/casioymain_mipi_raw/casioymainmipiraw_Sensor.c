 ...
 *============================================================================
 ****************************************************************************/
#include "casioymainmipiraw_Sensor.h"

#define CASIOYMAIN_EEPROM_READ_ID	0xA1
#define CASIOYMAIN_EEPROM_WRITE_ID	0xA0
#define CASIOYMAIN_MAX_OFFSET		0x8000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "casioymain_camera_sensor"
...
#define CASIOYMAIN_EEPROM_HEAD_ID     0x016B012B
#define CASIOYMAIN2_EEPROM_HEAD_ID    0x01C3012B
//#define CASIOYMAIN_UNIQUE_SENSOR_ID 0x0A1F
//#define CASIOYMAIN_UNIQUE_SENSOR_ID_LENGHT 11
...
static bool casioymain_probe_eeprom(struct subdrv_ctx *ctx);
static int casioymain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
//static void casioymain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int casioymain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void get_sensor_cali(void* arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
/* STRUCT */

static BYTE casioymain_common_data[OPLUS_CAMERA_COMMON_DATA_LENGTH] = { 0 };
//static BYTE casioymain_unique_id[CASIOYMAIN_UNIQUE_SENSOR_ID_LENGHT] = { 0 };
...
static struct comp_mode_tran_time_params casioymain_comp_params[SENSOR_SCENARIO_ID_MAX] = {
	{ .enable = 0, }, /*pre*/
...
static struct eeprom_map_info casioymain_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, casioymain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, casioymain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, casioymain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_DATA, casioymain_get_sensor_sn},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, casioymain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, casioymain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, casioymain_get_otp_checksum_data},
	//{SENSOR_FEATURE_GET_UNIQUE_SENSORID, casioymain_get_unique_sensorid},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, casioymain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, casioymain_set_awb_gain},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info_full = {
	...
	.i4Crop = {
		...
		/* <cust6> <cust7> <cust8> <cust9> <cust10>*/
		{2048, 1536}, {0, 384}, {0, 384}, {0, 0}, {0, 0}, {2048, 1536},
	},
	.iMirrorFlip = 3,
	.i4FullRawW = 8192,
	.i4FullRawH = 6144,
...
static struct subdrv_mode_struct mode_struct[] = {
	{/*Reg_A_QBIN(VBIN)_4096x3072_30FPS with PDAF VB_max*/
		...
		.mode_setting_table = casioymain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = casioymain_seamless_normal_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(casioymain_seamless_normal_preview),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3900,
	...
	{/*Reg_A_QBIN(VBIN)_4096x3072_30FPS with PDAF VB_max*/
		...
		.mode_setting_table = casioymain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_capture_setting),
	...
	{/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_A-1_QBIN(VBIN)_4096x3072_24FPS with PDAF VB_max**/
		...
		.mode_setting_table = casioymain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom2_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = casioymain_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(casioymain_seamless_custom2),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 4876,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom4_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = casioymain_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(casioymain_seamless_custom4),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3900,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom5_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = casioymain_seamless_custom5,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(casioymain_seamless_custom5),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3252,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom6_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = casioymain_seamless_custom6,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(casioymain_seamless_custom6),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3252,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = casioymain_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(casioymain_custom11_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = casioymain_seamless_custom11,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(casioymain_seamless_custom11),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 4064,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CASIOYMAIN_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = casioymain_ana_gain_table,
	.ana_gain_table_size = sizeof(casioymain_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = casioymain_init_setting,
	.init_setting_len = ARRAY_SIZE(casioymain_init_setting),
	.mode = mode_struct,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_PDN, 0, 3},
	{HW_ID_AVDD, 2812500, 3},
	{HW_ID_AFVDD, 2800000, 3},
	{HW_ID_DVDD, 1110000, 4},
	//{HW_ID_DOVDD, 1800000, 3},
	{HW_ID_RST, 1, 2},
	{HW_ID_MCLK_DRIVING_CURRENT, 4, 11},
};
...
const struct subdrv_entry casioymain_mipi_raw_entry = {
	.name = "casioymain_mipi_raw",
	.id = CASIOYMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_casioymain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != casioymain_eeprom_info[meta_id].meta)
		return -1;

	if (size != casioymain_eeprom_info[meta_id].size)
		return -1;

	addr = casioymain_eeprom_info[meta_id].start;
	readsize = casioymain_eeprom_info[meta_id].size;
...
static int casioymain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(feature_return_para_32, casioymain_common_data,
...
static void casioymain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(feature_return_para_32, casioymain_unique_id,
		CASIOYMAIN_UNIQUE_SENSOR_ID_LENGHT);
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CASIOYMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CASIOYMAIN_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CASIOYMAIN_EEPROM_READ_ID >> 1, reg, (CASIOYMAIN_EEPROM_WRITE_ID & 0xFE) | 0x01);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CASIOYMAIN_EEPROM_READ_ID >> 1, reg, CASIOYMAIN_EEPROM_WRITE_ID & 0xFE);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == CASIOYMAIN_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
            && (data_base == CASIOYMAIN_STEREO_START_ADDR )) {
            ...
            */
        } else if ((pStereodata->uSensorId == CASIOYMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == CASIOYMAIN_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, CASIOYMAIN_AESYNC_START_ADDR+7));
        ...
        LOG_INF("casioymain write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int casioymain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
    ...
    read_casioymain_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
            (BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CASIOYMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "casioymain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "casioymain read_otp_info end\n");
}
...
static u16 casioymain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	if (ctx->current_scenario_id == SENSOR_SCENARIO_ID_CUSTOM3 || // RMSC
		...
		casioymain_feedback_awbgain[5] = r_gain_int;
		casioymain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
		casioymain_feedback_awbgain[9] = b_gain_int;
		casioymain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
		subdrv_i2c_wr_regs_u8(ctx, casioymain_feedback_awbgain,
			ARRAY_SIZE(casioymain_feedback_awbgain));
	}
}
...
static bool casioymain_probe_eeprom(struct subdrv_ctx *ctx)
{
	...
	for (idx = 0; idx < eeprom_num; idx++) {
		...
		if (header_id == CASIOYMAIN_EEPROM_HEAD_ID || header_id == CASIOYMAIN2_EEPROM_HEAD_ID) {
...
static void set_sensor_cali(void *arg)
{
	...
	if (!casioymain_probe_eeprom(ctx))
		return;
...
void casioymain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int casioymain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	casioymain_get_min_shutter_by_scenario(ctx,
...
static void comp_mode_tran_time_cal1(struct subdrv_ctx *ctx, u32 scenario_id, u32* prsh) {
	...
	*prsh = 0U;
	if (casioymain_comp_params[ctx->current_scenario_id].clock_vtpxck == 0) {
	...
	system_delay = SYSTEM_DELAY1 * 1000 * 1000 * 10 /
		casioymain_comp_params[ctx->current_scenario_id].clock_vtpxck;
