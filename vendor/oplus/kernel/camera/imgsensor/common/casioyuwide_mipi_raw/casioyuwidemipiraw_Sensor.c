 ...
 *============================================================================
 ****************************************************************************/
#include "casioyuwidemipiraw_Sensor.h"

#define SENSOR_NAME  SENSOR_DRVNAME_CASIOYUWIDE_MIPI_RAW
#define PFX "casioyuwide_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

#define CASIOYUWIDE_EEPROM_I2C_ADDR	(0xA2)

#define OTP_SIZE	          (0x2000)
#define SENSOR_ID	          (0x56084700)

#define CASIOYUWIDE_STEREO_START_ADDR    (0x1260)
#define CASIOYUWIDE_AESYNC_START_ADDR    (0x1A00)

#define CASIOYUWIDE_UNIQUE_SENSOR_ID_ADDR 0x00
#define CASIOYUWIDE_UNIQUE_SENSOR_ID_LENGTH 16
static BYTE casioyuwide_unique_id[CASIOYUWIDE_UNIQUE_SENSOR_ID_LENGTH] = { 0 };

static struct subdrv_ctx *g_ctx = NULL;
static kal_uint8 otp_data_checksum[OTP_SIZE] = {0};
static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int casioyuwide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);

static int casioyuwide_set_gain_convert(struct subdrv_ctx *ctx, u32 gain);
static int casioyuwide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter);
static int casioyuwide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static int casioyuwide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data, int size);
static int casioyuwide_set_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_get_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int casioyuwide_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioyuwide_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void read_unique_sensorid(struct subdrv_ctx *ctx);

static int casioyuwide_common_control(struct subdrv_ctx *ctx,
...
static struct eeprom_map_info casioyuwide_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, casioyuwide_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, casioyuwide_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, casioyuwide_get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, casioyuwide_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, casioyuwide_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, casioyuwide_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, casioyuwide_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, casioyuwide_streaming_resume},
	{SENSOR_FEATURE_SET_GAIN, casioyuwide_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, casioyuwide_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, casioyuwide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_SEAMLESS_EXTEND_FRAME_LENGTH, casioyuwide_extend_frame_length},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, casioyuwide_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, casioyuwide_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_REGISTER, casioyuwide_set_register},
	{SENSOR_FEATURE_GET_REGISTER, casioyuwide_get_register},
	{SENSOR_FEATURE_SET_FRAMELENGTH, casioyuwide_set_frame_length},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{ /* 3264x2448 @30FPS  */
		...
		.mode_setting_table = casioyuwide_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_preview_capture_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = casioyuwide_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = casioyuwide_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = casioyuwide_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_slim_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 288000000,
...

	{
		...
		.mode_setting_table = casioyuwide_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = casioyuwide_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = casioyuwide_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = casioyuwide_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_custom4_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = casioyuwide_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(casioyuwide_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CASIOYUWIDE_SENSOR_ID,
	...
	.ana_gain_step = 1,
/*	.ana_gain_table = casioyuwide_ana_gain_table,
	.ana_gain_table_size = sizeof(casioyuwide_ana_gain_table),*/
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
...
/*	.reg_addr_fast_mode = 0x3010,*/
	...
	.init_setting_table = casioyuwide_init_setting,
	.init_setting_len = ARRAY_SIZE(casioyuwide_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	...
	.control = casioyuwide_common_control,
...
struct subdrv_entry casioyuwide_mipi_raw_entry = {
	.name = SENSOR_NAME,
	.id = CASIOYUWIDE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int casioyuwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			casioyuwide_set_shutter_convert(ctx, *(u32 *)para);
...
static unsigned int read_casioyuwide_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != casioyuwide_eeprom_info[meta_id].meta)
		return -1;

	if (size != casioyuwide_eeprom_info[meta_id].size)
		return -1;

	addr = casioyuwide_eeprom_info[meta_id].start;
	readsize = casioyuwide_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CASIOYUWIDE_EEPROM_I2C_ADDR >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
		...
		kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, CASIOYUWIDE_EEPROM_I2C_ADDR >> 1,
			addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, CASIOYUWIDE_EEPROM_I2C_ADDR >> 1, reg, 0x03);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, CASIOYUWIDE_EEPROM_I2C_ADDR >> 1, reg, 0x02);
	}
			...
			ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == CASIOYUWIDE_SENSOR_ID)
			&& (data_length == CALI_DATA_SLAVE_LENGTH)
			&& (data_base == CASIOYUWIDE_STEREO_START_ADDR)) {
		...
		} else if ((pStereodata->uSensorId == CASIOYUWIDE_SENSOR_ID)
			&& (data_length < AESYNC_DATA_LENGTH_TOTAL)
			&& (data_base == CASIOYUWIDE_AESYNC_START_ADDR)) {
			...
			LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR),
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR+1),
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR+2),
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR+3),
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR+4),
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR+5),
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR+6),
					read_cmos_eeprom_8(ctx, CASIOYUWIDE_AESYNC_START_ADDR+7));
...
static int casioyuwide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_casioyuwide_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
					BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CASIOYUWIDE_EEPROM_I2C_ADDR >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "casioyuwide read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "casioyuwide read_otp_info end\n");
}
...
static u16 casioyuwide_unique_sensorid[] = {
	0xfd, 0x00,
	0x20, 0x0f,
	0xe7, 0x03,
	0xe7, 0x00,
...
static void read_unique_sensorid(struct subdrv_ctx *ctx)
{
	...
	subdrv_i2c_wr_regs_u8_u8(ctx, casioyuwide_unique_sensorid, ARRAY_SIZE(casioyuwide_unique_sensorid));
	msleep(50);
	subdrv_i2c_wr_u8_u8(ctx, 0xfd, 0x08);
	for (i = 0; i< CASIOYUWIDE_UNIQUE_SENSOR_ID_LENGTH; i++) {
		casioyuwide_unique_id[i] =  subdrv_i2c_rd_u8_u8(ctx, CASIOYUWIDE_UNIQUE_SENSOR_ID_ADDR + i);
		pr_err("%s unique_id[%d] = 0x%x", __func__, i, casioyuwide_unique_id[i]);
	}
}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	/* initail setting */
	subdrv_i2c_wr_regs_u8_u8(ctx, casioyuwide_soft_reset, ARRAY_SIZE(casioyuwide_soft_reset));
	mdelay(3);
	subdrv_i2c_wr_regs_u8_u8(ctx, casioyuwide_init_setting, ARRAY_SIZE(casioyuwide_init_setting));
...
void casioyuwide_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
	u32 fll_step = 0;
	u32 vblank = 0;
...
void casioyuwide_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
		enum SENSOR_SCENARIO_ID_ENUM scenario_id,
...
static int casioyuwide_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	casioyuwide_write_frame_length(ctx, ctx->frame_length);
	if (gph) {
...
static int casioyuwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return casioyuwide_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
}

static int casioyuwide_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter)
{
	return casioyuwide_set_shutter_frame_length_convert(ctx, shutter, 0);
}

static int casioyuwide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return casioyuwide_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], 0);
}
...
static int casioyuwide_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return casioyuwide_set_shutter_frame_length_convert(ctx, shutters[0], framelength);
}
...
static int casioyuwide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return casioyuwide_set_gain_convert(ctx, gain);
}

void casioyuwide_set_dummy(struct subdrv_ctx *ctx)
{
}

static void casioyuwide_set_mirror_flip(struct subdrv_ctx *ctx, kal_uint8 image_mirror)
{
	u8 itemp = 0;
	subdrv_i2c_wr_u8_u8(ctx, 0xfd, 0x01);
	itemp = subdrv_i2c_rd_u8_u8(ctx, ctx->s_ctx.reg_addr_mirror_flip) & ~0x03;
	...
	subdrv_i2c_wr_u8_u8(ctx, ctx->s_ctx.reg_addr_mirror_flip, itemp);
}
...
static int casioyuwide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.reg_addr_auto_extend ||
		...
		casioyuwide_set_dummy(ctx);
	}

	return 0;
}

static int casioyuwide_extend_frame_length_convert(struct subdrv_ctx *ctx, u32 ns)
{
	return 0;
}
...
static int casioyuwide_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return casioyuwide_extend_frame_length_convert(ctx, ns);
}
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt)
{
...
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id) {
	struct subdrv_entry *m_subdrv_entry = &casioyuwide_mipi_raw_entry;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	casioyuwide_set_mirror_flip(ctx, ctx->s_ctx.mirror);
	return ret;
}
