 ...
 *============================================================================
 ****************************************************************************/
#include "hondawidemipiraw_Sensor.h"
#define SENSOR_NAME  SENSOR_DRVNAME_HONDAWIDE_MIPI_RAW
#define PFX "hondawide_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

#define HONDAWIDE_EEPROM_I2C_ADDR	(0xA2)

#define OTP_SIZE	          (0x2000)
#define SENSOR_ID	          (0x56084700)

#define HONDAWIDE_STEREO_START_ADDR    (0x1260)
#define HONDAWIDE_AESYNC_START_ADDR    (0x1A00)
#define HONDAWIDE_UNIQUE_SENSOR_ID_ADDR 0x00
#define HONDAWIDE_UNIQUE_SENSOR_ID_LENGTH 16
static BYTE hondawide_unique_id[HONDAWIDE_UNIQUE_SENSOR_ID_LENGTH] = { 0 };

static struct subdrv_ctx *g_ctx = NULL;
static kal_uint8 otp_data_checksum[OTP_SIZE] = {0};
static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int hondawide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);

static int hondawide_set_gain_convert(struct subdrv_ctx *ctx, u32 gain);
static int hondawide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter);
static int hondawide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static int hondawide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data, int size);
static int hondawide_set_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_get_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt);
static int hondawide_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_common_control(struct subdrv_ctx *ctx,
...
static int hondawide_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hondawide_get_cloud_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static struct eeprom_map_info hondawide_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, hondawide_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, hondawide_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, hondawide_get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, hondawide_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, hondawide_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, hondawide_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, hondawide_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, hondawide_streaming_resume},
	{SENSOR_FEATURE_SET_GAIN, hondawide_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, hondawide_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, hondawide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_SEAMLESS_EXTEND_FRAME_LENGTH, hondawide_extend_frame_length},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, hondawide_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, hondawide_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_REGISTER, hondawide_set_register},
	{SENSOR_FEATURE_GET_REGISTER, hondawide_get_register},
	{SENSOR_FEATURE_SET_FRAMELENGTH, hondawide_set_frame_length},
	{SENSOR_FEATURE_GET_UNIQUE_SENSORID, hondawide_get_unique_sensorid},
	{SENSOR_FEATURE_GET_CLOUD_OTP_INFO, hondawide_get_cloud_otp_info},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{ /* 3264x2448 @30FPS  */
		...
		.mode_setting_table = hondawide_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_preview_capture_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = hondawide_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = hondawide_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = hondawide_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_slim_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 288000000,
...

	{
		...
		.mode_setting_table = hondawide_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = hondawide_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = hondawide_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = hondawide_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_custom4_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = hondawide_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(hondawide_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_step = 1,
	.ana_gain_table = hondawide_ana_gain_table,
	.ana_gain_table_size = sizeof(hondawide_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	.exposure_max =  0x23FFFF,
	.exposure_step = 4,
	.exposure_margin = 20,

	.frame_length_max = 0x23FFFF,
	.ae_effective_frame = 2,
	.frame_time_delay_frame = 2,
	.start_exposure_offset = 1834000,
...
/*	.reg_addr_fast_mode = 0x3010,*/
	...
	.init_setting_table = hondawide_init_setting,
	.init_setting_len = ARRAY_SIZE(hondawide_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	...
	.control = hondawide_common_control,
...
struct subdrv_entry hondawide_mipi_raw_entry = {
	.name = SENSOR_NAME,
	.id = HONDAWIDE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int hondawide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			hondawide_set_shutter_convert(ctx, *(u32 *)para);
...
static unsigned int read_hondawide_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != hondawide_eeprom_info[meta_id].meta)
		return -1;

	if (size != hondawide_eeprom_info[meta_id].size)
		return -1;

	addr = hondawide_eeprom_info[meta_id].start;
	readsize = hondawide_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, HONDAWIDE_EEPROM_I2C_ADDR >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
		...
		kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, HONDAWIDE_EEPROM_I2C_ADDR >> 1,
			addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, HONDAWIDE_EEPROM_I2C_ADDR >> 1, reg, 0x03);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, HONDAWIDE_EEPROM_I2C_ADDR >> 1, reg, 0x02);
	}
			...
			ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == HONDAWIDE_SENSOR_ID || pStereodata->uSensorId == MILKYWAYS2WIDE_SENSOR_ID)
			&& (data_length == CALI_DATA_SLAVE_LENGTH) && (data_base == HONDAWIDE_STEREO_START_ADDR)) {
		...
		} else if ((pStereodata->uSensorId == HONDAWIDE_SENSOR_ID || pStereodata->uSensorId == MILKYWAYS2WIDE_SENSOR_ID)
			&& (data_length < AESYNC_DATA_LENGTH_TOTAL) && (data_base == HONDAWIDE_AESYNC_START_ADDR)) {
			...
			LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR),
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR+1),
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR+2),
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR+3),
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR+4),
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR+5),
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR+6),
					read_cmos_eeprom_8(ctx, HONDAWIDE_AESYNC_START_ADDR+7));
...
static int hondawide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_hondawide_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
					BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, HONDAWIDE_EEPROM_I2C_ADDR >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "hondawide read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "hondawide read_otp_info end\n");
}
...
static u16 hondawide_unique_sensorid[] = {
	0xfd, 0x00,
	0x20, 0x0f,
	0xe7, 0x03,
	0xe7, 0x00,
...
{
	u32 *feature_return_para_32 = (u32 *)para;
	*len = HONDAWIDE_UNIQUE_SENSOR_ID_LENGTH;
	memcpy(feature_return_para_32, hondawide_unique_id,
		HONDAWIDE_UNIQUE_SENSOR_ID_LENGTH);
...
static void read_unique_sensorid(struct subdrv_ctx *ctx)
{
	...
	subdrv_i2c_wr_regs_u8_u8(ctx, hondawide_unique_sensorid, ARRAY_SIZE(hondawide_unique_sensorid));
	msleep(50);
	subdrv_i2c_wr_u8_u8(ctx, 0xfd, 0x08);
	for (i = 0; i < HONDAWIDE_UNIQUE_SENSOR_ID_LENGTH; i++) {
		hondawide_unique_id[i] =  subdrv_i2c_rd_u8_u8(ctx, HONDAWIDE_UNIQUE_SENSOR_ID_ADDR + i);
	}
}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	/* initail setting */
	subdrv_i2c_wr_regs_u8_u8(ctx, hondawide_soft_reset, ARRAY_SIZE(hondawide_soft_reset));
	mdelay(3);
	subdrv_i2c_wr_regs_u8_u8(ctx, hondawide_init_setting, ARRAY_SIZE(hondawide_init_setting));
...
void hondawide_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
	u32 fll_step = 0;
	u32 vblank = 0;
...
void hondawide_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
		enum SENSOR_SCENARIO_ID_ENUM scenario_id,
...
static int hondawide_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	hondawide_write_frame_length(ctx, ctx->frame_length);
	if (gph)
...
static int hondawide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return hondawide_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
}

static int hondawide_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter)
{
	return hondawide_set_shutter_frame_length_convert(ctx, shutter, 0);
}

static int hondawide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return hondawide_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], 0);
}
...
static int hondawide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return hondawide_set_gain_convert(ctx, gain);
}

void hondawide_set_dummy(struct subdrv_ctx *ctx)
{
}
...
static int hondawide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	DRV_LOG(ctx, "scenario_id(%d), framerate(%d)", scenario_id, framerate);
	...
	if (ctx->s_ctx.reg_addr_auto_extend ||
		...
		hondawide_set_dummy(ctx);
	}

	return 0;
}

static void hondawide_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
{
	u64* feature_data = (u64*)para;
	hondawide_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static int hondawide_extend_frame_length_convert(struct subdrv_ctx *ctx, u32 ns)
{
	return 0;
}
...
static int hondawide_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return hondawide_extend_frame_length_convert(ctx, ns);
}
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt)
{
...
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id) {
	struct subdrv_entry *m_subdrv_entry = &hondawide_mipi_raw_entry;
