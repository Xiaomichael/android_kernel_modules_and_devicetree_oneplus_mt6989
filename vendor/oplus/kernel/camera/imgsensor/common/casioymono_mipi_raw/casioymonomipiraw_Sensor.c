 ...
 *============================================================================
 ****************************************************************************/
#include "casioymonomipiraw_Sensor.h"

#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "casioymono_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

static void casioymono_sensor_init(struct subdrv_ctx *ctx);
static u16 get_gain2reg(u32 gain);
static int casioymono_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymono_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymono_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int casioymono_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymono_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymono_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymono_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymono_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int casioymono_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void casioymono_write_frame_length(struct subdrv_ctx *ctx, u32 fll);
/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, casioymono_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, casioymono_check_sensor_id},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, casioymono_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_GAIN, casioymono_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, casioymono_set_shutter},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, casioymono_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, casioymono_streaming_resume},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, casioymono_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, casioymono_set_multi_shutter_frame_length_ctrl},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = casioymono_preview_setting,
		.mode_setting_len = ARRAY_SIZE(casioymono_preview_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 72000000,
...

	{
		...
		.mode_setting_table = casioymono_capture_setting,
		.mode_setting_len = ARRAY_SIZE(casioymono_capture_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 72000000,
...

	{
		...
		.mode_setting_table = casioymono_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioymono_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 72000000,
...

	{
		...
		.mode_setting_table = casioymono_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioymono_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 72000000,
...

	{
		...
		.mode_setting_table = casioymono_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(casioymono_slim_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 72000000,
...

	{
		...
		.mode_setting_table = casioymono_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(casioymono_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 72000000,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CASIOYMONO_SENSOR_ID,
	...
	.ana_gain_type = 1,
	.ana_gain_step = 1,
	.ana_gain_table = casioymono_ana_gain_table,
	.ana_gain_table_size = sizeof(casioymono_ana_gain_table),
	.min_gain_iso = 50,
	.exposure_def = 0x3D0,
	.exposure_min = 2,
	...
	.init_setting_table = casioymono_init_setting,
	.init_setting_len = ARRAY_SIZE(casioymono_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry casioymono_mipi_raw_entry = {
	.name = "casioymono_mipi_raw",
	.id = CASIOYMONO_SENSOR_ID,
	.pw_seq = pw_seq,
	.pw_seq_cnt = ARRAY_SIZE(pw_seq),
	.ops = &ops,
};
static void casioymono_set_dummy(struct subdrv_ctx *ctx)
{
...
static void casioymono_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
			kal_bool min_framelength_en)
{
	...
	casioymono_set_dummy(ctx);
}
static void casioymono_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps > 297 && realtime_fps <= 305) {
			casioymono_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps > 147 && realtime_fps <= 150) {
			casioymono_set_max_framerate(ctx, 146, 0);
		} else {
			/* Extend frame length */
			subdrv_i2c_wr_u8(ctx, 0x320e, ctx->frame_length >> 8);
			subdrv_i2c_wr_u8(ctx, 0x320f, ctx->frame_length & 0xff);
...
static int casioymono_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	casioymono_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}
/* FUNCTION */
static void casioymono_write_shutter(struct subdrv_ctx *ctx)
{
...
static void casioymono_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	casioymono_write_shutter(ctx);
}

static int casioymono_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *para);
	casioymono_set_shutter_convert(ctx, (u32 *)para);
	return 0;
}
...
static void casioymono_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
		u32 *shutters, u16 exp_cnt,	u16 frame_length)
{
	...
	if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
		casioymono_write_frame_length(ctx, ctx->frame_length);
	/* write shutter */
	switch (exp_cnt) {
	case 1:
...
static int casioymono_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	casioymono_set_multi_shutter_frame_length(ctx, (u32 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

void casioymono_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
static void casioymono_sensor_init(struct subdrv_ctx *ctx)
{
	...
	subdrv_i2c_wr_u8(ctx, 0x3221, 0x00);
}
...
void casioymono_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int casioymono_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	casioymono_get_min_shutter_by_scenario(ctx,
