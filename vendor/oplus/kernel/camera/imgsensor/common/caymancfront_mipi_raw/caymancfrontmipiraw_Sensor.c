 ...
 *============================================================================
 ****************************************************************************/
#include "caymancfrontmipiraw_Sensor.h"

#define PFX "caymancfront_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

#define CAYMANCFRONT_EEPROM_READ_ID			0xA9
#define CAYMANCFRONT_EEPROM_WRITE_ID			0xA8
#define CAYMANCFRONT_MAX_OFFSET				0x4000
...
static int caymancfront_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymancfront_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void get_sensor_cali(void* arg);
static void set_sensor_cali(void *arg);
static int caymancfront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct eeprom_map_info caymancfront_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, caymancfront_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, caymancfront_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, caymancfront_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, caymancfront_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, caymancfront_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, caymancfront_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, caymancfront_streaming_resume},
	{SENSOR_FEATURE_SET_ESHUTTER, caymancfront_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, caymancfront_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_GAIN, caymancfront_set_gain},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = caymancfront_preview_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_preview_setting),
	...
	},
	{
		...
		.mode_setting_table = caymancfront_capture_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = caymancfront_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = caymancfront_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = caymancfront_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = caymancfront_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = caymancfront_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = caymancfront_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = caymancfront_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(caymancfront_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CAYMANCFRONT_SENSOR_ID,
	...
	.ana_gain_step = 1,
	.ana_gain_table = caymancfront_ana_gain_table,
	.ana_gain_table_size = sizeof(caymancfront_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = caymancfront_sensor_init_setting,
	.init_setting_len =  ARRAY_SIZE(caymancfront_sensor_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry caymancfront_mipi_raw_entry = {
	.name = "caymancfront_mipi_raw",
	.id = CAYMANCFRONT_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void caymancfront_set_dummy(struct subdrv_ctx *ctx)
{
...
static void caymancfront_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
			kal_bool min_framelength_en)
{
	...
	caymancfront_set_dummy(ctx);
}

static void caymancfront_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305) {
			caymancfront_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps >= 147 && realtime_fps <= 150) {
			caymancfront_set_max_framerate(ctx, 146, 0);
		} else {
			// Extend frame length
			subdrv_i2c_wr_u8(ctx, 0x0104, 0x01);
			subdrv_i2c_wr_u8(ctx, 0x0340, ctx->frame_length >> 8);
...
static int caymancfront_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	caymancfront_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}

static void caymancfront_write_shutter(struct subdrv_ctx *ctx)
{
	kal_uint16 realtime_fps = 0;
...
static void caymancfront_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	caymancfront_write_shutter(ctx);
}

static int caymancfront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *para);
	caymancfront_set_shutter_convert(ctx, (u32 *)para);
	return 0;
}
...
static unsigned int read_caymancfront_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != caymancfront_eeprom_info[meta_id].meta)
		return -1;

	if (size != caymancfront_eeprom_info[meta_id].size)
		return -1;

	addr = caymancfront_eeprom_info[meta_id].start;
	readsize = caymancfront_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
    ...
    adaptor_i2c_rd_u8(ctx->i2c_client, CAYMANCFRONT_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
    return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CAYMANCFRONT_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANCFRONT_EEPROM_WRITE_ID >> 1, reg, 0xA1);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANCFRONT_EEPROM_WRITE_ID >> 1, reg, 0xA0);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == CAYMANCFRONT_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
            && (data_base == CAYMANCFRONT_STEREO_START_ADDR)) {
        ...
        } else if ((pStereodata->uSensorId == CAYMANCFRONT_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == CAYMANCFRONT_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, CAYMANCFRONT_AESYNC_START_ADDR+7));
        ...
        LOG_INF("caymancfront write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int caymancfront_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_caymancfront_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
			(BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CAYMANCFRONT_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "caymancfront read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "caymancfront read_otp_info end\n");
}
...
int caymancfront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
