 ...
 *============================================================================
 ****************************************************************************/
#include "baikalmmonomipiraw_Sensor.h"

// #define BAIKALMMONO_EEPROM_READ_ID	0xA5
// #define BAIKALMMONO_EEPROM_WRITE_ID   0xA4
#define BAIKALMMONO_MAX_OFFSET		0x1400
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "baikalmmono_camera_sensor"
...
static int baikalmmono_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
// static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
// static int baikalmmono_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int close(struct subdrv_ctx *ctx);
static int baikalmmono_control(struct subdrv_ctx *ctx,
...
//                     BYTE *data, int size);
static int baikalmmono_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int baikalmmono_set_mirror_filp(struct subdrv_ctx *ctx, u8 image_mirror);
static void milkywayc2mono_write_frame_length(struct subdrv_ctx *ctx, u32 fll);
/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, baikalmmono_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, baikalmmono_check_sensor_id},
	// {SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	// {SENSOR_FEATURE_GET_SENSOR_OTP_ALL, baikalmmono_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_ESHUTTER, baikalmmono_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, baikalmmono_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, baikalmmono_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, baikalmmono_streaming_resume},
	{SENSOR_FEATURE_SET_GAIN, baikalmmono_set_gain},
	{SENSOR_FEATURE_SET_FRAMELENGTH, baikalmmono_set_frame_length},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, baikalmmono_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, baikalmmono_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, baikalmmono_set_multi_shutter_frame_length_ctrl},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = baikalmmono_preview_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmono_preview_setting),
	...
	},
	{
		...
		.mode_setting_table = baikalmmono_capture_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmono_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmmono_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmono_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmmono_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmono_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmmono_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmono_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = baikalmmono_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(baikalmmono_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = BAIKALMMONO_SENSOR_ID,
	...
	.ana_gain_step = 2,
	.ana_gain_table = baikalmmono_ana_gain_table,
	.ana_gain_table_size = sizeof(baikalmmono_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	// .init_setting_table = baikalmmono_sensor_init_setting,
	// .init_setting_len =  ARRAY_SIZE(baikalmmono_sensor_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry baikalmmono_mipi_raw_entry = {
	.name = "baikalmmono_mipi_raw",
	.id = BAIKALMMONO_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void baikalmmono_set_dummy(struct subdrv_ctx *ctx)
{
...
static void baikalmmono_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
			kal_bool min_framelength_en)
{
	...
	baikalmmono_set_dummy(ctx);
}

static void baikalmmono_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps > 592 && realtime_fps <= 607) {
			baikalmmono_set_max_framerate(ctx, 592, 0);
		} else if (realtime_fps > 296 && realtime_fps <= 305) {
			baikalmmono_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps > 246 && realtime_fps <= 253) {
			baikalmmono_set_max_framerate(ctx, 246, 0);
		} else if (realtime_fps > 236 && realtime_fps <= 243) {
			baikalmmono_set_max_framerate(ctx, 236, 0);
		} else if (realtime_fps > 146 && realtime_fps <= 153) {
			baikalmmono_set_max_framerate(ctx, 146, 0);
		} else {
			/* Extend frame length */
			subdrv_i2c_wr_u8_u8(ctx, 0xfd, 0x01);
			subdrv_i2c_wr_u8_u8(ctx, 0x14, (ctx->frame_length - 1220) >> 8);
...
static int baikalmmono_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmmono_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}

static void baikalmmono_write_shutter(struct subdrv_ctx *ctx)
{
	kal_uint16 realtime_fps = 0;
...
static void baikalmmono_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	baikalmmono_write_shutter(ctx);
}

static int baikalmmono_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *para);
	baikalmmono_set_shutter_convert(ctx, (u32 *)para);
	return 0;
}
...
//                     BYTE *data, int size)
// {
// 	if (adaptor_i2c_rd_p8(ctx->i2c_client, BAIKALMMONO_EEPROM_READ_ID >> 1,
// 			addr, data, size) < 0) {
// 		return false;
// 	}
// 	return true;
...
static int open(struct subdrv_ctx *ctx)
{
	...
	//sensor_init(ctx);
	/* initail setting */
	subdrv_i2c_wr_regs_u8_u8(ctx, baikalmmono_soft_reset, ARRAY_SIZE(baikalmmono_soft_reset));
	mdelay(5);
	subdrv_i2c_wr_regs_u8_u8(ctx, baikalmmono_sensor_init_setting, ARRAY_SIZE(baikalmmono_sensor_init_setting));

	baikalmmono_set_mirror_filp(ctx, ctx->s_ctx.mirror);
...
int baikalmmono_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
...
static void baikalmmono_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
static int baikalmmono_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmmono_set_multi_shutter_frame_length(ctx, (u32 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static void baikalmmono_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u32 values[3] = {0};
	...
	baikalmmono_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int baikalmmono_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmmono_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int baikalmmono_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	baikalmmono_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}
