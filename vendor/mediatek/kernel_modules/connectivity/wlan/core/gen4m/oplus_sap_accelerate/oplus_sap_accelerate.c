...
static long g_all_pkt_num;
static long g_accelerated_pkt_num;
...
static bool oplus_checkif_ip_addr_private(__be32 ipaddr)
{
...
static void add_new_linklist_node(__be32 server_ip_addr, __be32 sta_ip_addr)
{
    struct prior_ip_addr_node *new_node =
            kmalloc(sizeof(struct prior_ip_addr_node), GFP_ATOMIC);
    if (!new_node) {
        DBGLOG(RX, ERROR, "[oplusSapAccelerate]assign new node failed\n");
        return;
    }
...
{
    if (!g_inited) {
        oplus_sap_accelerate_module_init();
    }
    g_func_enable = 1;
}
...
{
    if (g_inited) {
        oplus_sap_accelerate_module_deinit();
    }
    g_func_enable = 0;
}
...
void oplus_get_sap_acc_statistic_data(long *statistic_data)
{
    if (!statistic_data) {
        return;
    }
    statistic_data[0] = g_all_pkt_num;
    statistic_data[1] = g_accelerated_pkt_num;
}

void oplus_nic_rx_mark_prior_pkt(struct SW_RFB *prRetSwRfb)
{
    if (!g_func_enable) {
        return;
    }
...
void oplus_kal_rx_save_prior_pkt_info(struct sk_buff *prSkb)
{
    if (!g_func_enable) {
        return;
    }
    ...
    if (oplus_checkif_ip_addr_private(daddr)) {
        return;
    }
    ...
    if (!is_high_prior && !is_found_in_list) {
        ...
        if (prior_ip_addr_list_cur_length >=
            OPLUS_PRIOR_IP_ADDR_LIST_MAX_LENGTH) {
            if (last) {
                hlist_del(&last->node);
                kfree(last);
            }
        }
        add_new_linklist_node(daddr, saddr);
        DBGLOG(RX, INFO,
            "[oplusSapAccelerate]new addr pair: server[%pI4] sta[%pI4]\n",
            &daddr, &saddr);
        goto unlock;
    }
    else {
...
void oplus_kal_tx_accelerate_prior_pkt(struct sk_buff *prSkb)
{
    if (!g_func_enable) {
        return;
    }
    ...
    spin_lock_bh(&prior_ip_addr_list_lock);
    if (g_all_pkt_num < LONG_MAX) {
        g_all_pkt_num++;
    }
    hlist_for_each_entry(entry, &prior_ip_addr_list_head, node) {
        if (!entry) {
            DBGLOG(RX, ERROR, "[oplusSapAccelerate]entry is null\n");
            continue;
        }
        if (entry->server_ip_addr == saddr && entry->sta_ip_addr == daddr) {
            if (entry->high_prior_count >= OPLUS_SAP_ACCELERATE_THRESHOLD) {
                if (g_all_pkt_num < LONG_MAX) {
                    g_accelerated_pkt_num++;
                }
#ifdef CONFIG_ANDROID_KABI_RESERVE
#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 12, 0))
