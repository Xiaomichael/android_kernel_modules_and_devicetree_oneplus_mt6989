				  ...
				  uint32_t u4Check)
{
	...
	if (!prGlueInfo)
		...
		if ((u4Check & WLAN_DRV_READY_CHECK_HIF_SUSPEND) &&
			...
			DBGLOG(REQ, INFO, "driver state[%d]\n", u1State);
			fgIsReady = FALSE;
		}
				  ...
				  int8_t *pTxPwr)
{
	...
	*pTxPwr = minTxPwr >> 1;
	...
	return WLAN_STATUS_SUCCESS;
#else
	DBGLOG(INIT, INFO, "Not supported!");
...
#undef __STR_MIN_RESERVED
	...
	INIT_UINT(prWifiVar->fgDisDriverRoaming,
		"DisDriverRoaming", 0);
	...
	INIT_UINT(prWifiVar->fgBoostCpuByPPSEn,  "BoostCpuByPPSEn", FEATURE_ENABLED);
	...
	INIT_UINT(prWifiVar->u4MlrCfg, "MlrCfg", 0);
#endif
