...
__priv_nan_struct(struct net_device *prNetDev,
	...
	case ENUM_NAN_PUBLISH: {
		...
		if (publishReq->scid_len <= NAN_MAX_SCID_BUF_LEN)
				    ...
				    char *pcCommand, int i4TotalLen)
{
	...
	if (pucMacAddr) {
		...
		rStatus = kalIoctlByBssIdx(prGlueInfo,
				...
				sizeof(rQueryStaStatistics), &u4BufLen,
				ucBssIndex);
		DBGLOG(REQ, INFO, "bss:%u wtbl:%u\n", ucBssIndex, ucWlanIndex);
				    ...
				    char *pcCommand, int i4TotalLen)
{
	...
	rStatus = kalIoctlByBssIdx(prGlueInfo, wlanoidQueryStaStatistics,
			prQueryStaStatistics,
				  ...
				  char *pcCommand, int i4TotalLen)
{
	...
	if (i4Argc >= 3) {
		...
		// TODO : switch case with u4Action for different handler
		if (u4Action == 0)
