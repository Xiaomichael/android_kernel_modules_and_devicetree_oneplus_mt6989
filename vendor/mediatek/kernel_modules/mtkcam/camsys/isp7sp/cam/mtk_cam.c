...
void mtk_cam_ctx_engine_off(struct mtk_cam_ctx *ctx)
{
	...
	if (ctx->hw_sv) {
		...
		/* ois compensation */
		if (ctx->cam->common_sv &&
				ctx->cam->common_sv->id == sv_dev->id) {
#ifdef OPLUS_FEATURE_CAMERA_COMMON
			mtk_cam_tuning_uninit();
#endif /* OPLUS_FEATURE_CAMERA_COMMON */
			disable_irq(ctx->cam->sv_common_irq);
			ctx->cam->common_sv = NULL;
		}
	}
...
int mtk_cam_reset_msgfifo(struct mtk_cam_device *cam_dev)
{
	atomic_set(&cam_dev->is_fifo_overflow, 0);
	return kfifo_init(&cam_dev->msg_fifo, cam_dev->msg_buffer, cam_dev->fifo_size);
}

static int push_msgfifo(struct mtk_cam_device *cam_dev,
			struct mtk_camsys_irq_info *info)
{
	int len;

	if (unlikely(kfifo_avail(&cam_dev->msg_fifo) < sizeof(*info))) {
		atomic_set(&cam_dev->is_fifo_overflow, 1);
		return -1;
	}

	len = kfifo_in(&cam_dev->msg_fifo, info, sizeof(*info));
...
{
	struct mtk_cam_device *cam_dev = (struct mtk_cam_device *)data;
	struct mtk_camsv_device *sv_dev = cam_dev->common_sv;
	struct mtk_camsys_irq_info irq_info;
	unsigned int frm_seq_no, frm_seq_no_inner;
	unsigned int i, first_tag, common_status, camctl_misc;
	...
	if (first_tag) {
		...
		if (common_status & BIT(exp_1_bid)) {
			if (sv_dev->is_skip_raw_unlock_done)
				...
				camctl_misc = readl_relaxed(sv_dev->raw_lock_done_sel);
				camctl_misc &= ~BIT(20);
				writel_relaxed(camctl_misc, sv_dev->raw_lock_done_sel);
				sv_dev->ois_updated_seq = frm_seq_no_inner;
			}
		}
	}

	if (irq_info.irq_type && push_msgfifo(cam_dev, &irq_info) == 0)
		wake_thread = true;

	return wake_thread ? IRQ_WAKE_THREAD : IRQ_HANDLED;
}

static irqreturn_t mtk_thread_irq_cam(int irq, void *data)
{
	struct mtk_cam_device *cam_dev = (struct mtk_cam_device *)data;
	struct mtk_camsys_irq_info irq_info;

	if (unlikely(atomic_cmpxchg(&cam_dev->is_fifo_overflow, 1, 0)))
		dev_info(cam_dev->dev, "msg fifo overflow\n");

	while (kfifo_len(&cam_dev->msg_fifo) >= sizeof(irq_info)) {
		int len = kfifo_out(&cam_dev->msg_fifo, &irq_info, sizeof(irq_info));

		WARN_ON(len != sizeof(irq_info));

		if (CAM_DEBUG_ENABLED(CTRL))
			dev_info(cam_dev->dev, "ts=%llu irq_type %d, req:0x%x/0x%x\n",
			irq_info.ts_ns / 1000,
			irq_info.irq_type,
			irq_info.frame_idx_inner,
			irq_info.frame_idx);

		mtk_cam_ctrl_isr_event(cam_dev,
				       CAMSYS_ENGINE_CAMSV, cam_dev->common_sv->id,
				       &irq_info);
	}
...
SKIP_ADLRD_IRQ:
	cam_dev->sv_common_irq = platform_get_irq(pdev, 2);
	if (cam_dev->sv_common_irq < 0) {
		dev_err(dev, "%s: failed to get sv common irq\n", __func__);
		goto SKIP_SV_COMMON_IRQ;
	}

	ret = devm_request_threaded_irq(dev, cam_dev->sv_common_irq,
			mtk_irq_camsv_debug,
			mtk_thread_irq_cam,
			IRQF_NO_AUTOEN, dev_name(dev), cam_dev);
	if (ret) {
		dev_err(dev, "failed to request irq=%d\n", cam_dev->sv_common_irq);
		return ret;
	}
	dev_info(dev, "registered irq=%d\n", cam_dev->sv_common_irq);

	cam_dev->common_sv = NULL;
	cam_dev->fifo_size =
		roundup_pow_of_two(8 * sizeof(struct mtk_camsys_irq_info));

	cam_dev->msg_buffer = devm_kzalloc(dev, cam_dev->fifo_size, GFP_KERNEL);
	if (!cam_dev->msg_buffer) {
		dev_err(dev, "failed to request msg buffer\n");
		return -ENOMEM;
	}
