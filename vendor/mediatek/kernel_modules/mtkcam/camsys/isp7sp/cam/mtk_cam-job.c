...
_stream_on(struct mtk_cam_job *job, bool on)
{
	...
	if (ctx->hw_sv) {
		...
		if (is_ois_compensation(job) && on) {
			ctx->cam->common_sv = sv_dev;
			mtk_cam_sv_exp_setup(sv_dev,
				get_tuning_begin_line(job), get_tuning_end_line(job));
			mtk_cam_reset_msgfifo(ctx->cam);
			enable_irq(ctx->cam->sv_common_irq);

			pr_info("%s: ois compensation sv id:%d\n", __func__, sv_dev->id);
		}
...
static int master_raw_set_normal(struct device *dev, struct mtk_cam_job *job)
{
